...
1. **양자 승인 대상 — 12곳:** `contracts/execution.py·llm.py·gates.py·evaluation.py·taxonomy.py·agents.py·graphrag.py`(★v5 — GraphRAG 경계 계약), `evidence/models.py`(EvidenceRef 스키마), `api/app.py·api/envelope.py`, `db/models.py·db/base.py`
   ...
   - 🔴 **(2026-08-25 신설 · B 승인) 문면 변경의 경계 — 이 줄이 정본이다**(99 #252). 다른 문서는 문면을 복제하지 말고 여기를 가리킨다(§4-1 의 「수의 정본」과 같은 원칙 · #240).
     > 양자 파일의 **문면 변경**:
     > · **규약·소유·승인 대상을 말하는 문장**은 주석이어도 **승인 대상**이다.
     > · **사실 진술을 참으로 고치는 것**은 승인 불요. 단 PR 설명에 «무엇이 거짓이었고 무엇이 참인가» 를 적고 상대 축에 통보한다.
     > · **스키마·시그니처·기본값·타입은 언제나 승인 대상**이다.
     > 🔴 판단이 안 서면 **승인 대상으로 본다.** 애매한 것을 넘기는 쪽이 규약을 녹인다.

     ⚠ 🔴 **추상만 두면 또 갈린다**(#252 가 그렇게 났다) — 실제 예를 둘 붙인다:
     - **규약을 말하는 문장(승인 대상)** — `db/models.py` 의 «⚠ 이 목록 밖이다 · 만들 때 편입을 협의한다(99 #245)» 계열. 🔴 **무엇이 승인 대상인가를 말하므로** 주석이어도 규약이다.
     - **사실을 말하는 문장(승인 불요)** — `db/models.py:LabelConfirmationStat` 의 «이 테이블은 «어느 축의 제안이 얼마나 뒤집히나» 를 센다» 계열. 🔴 **테이블이 무엇을 담는지**를 말할 뿐이고, 틀렸으면 참으로 고치는 것이 곧 문서의 일이다.
...
6. **(2026-08-08 신설) 미확정 안건의 추적 정본 — 99는 색인, 소유자 문서는 본문:**
   ...
   - `docs/part_b/09_integration_proposals.md`(B) 같은 **소유자 문서 = 본문.** 근거·실측·처방은 거기 살고 **99의 한 줄이 그리로 가리킨다.**
   - ⚠ **본문을 99에 복제하지 않는다**(§4-1의 「같은 목록이 두 곳」과 같은 원칙) — 색인 줄은 **제목 + 포인터**까지다.
   - 🔴 **왜 이 규약인가:** B가 *"PR 본문은 grep이 안 된다"* 를 등재 사유로 세웠는데, 그 논리를 한 칸 더 밀면 **소유자 문서는 grep은 되지만 색인이 아니다**가 된다. 색인이 갈리면 *"남은 게 몇 개냐"* 에 답할 수 있는 자리가 사라진다. **제안: B(2026-08-08) · 수용: A** — 규율 문서라 여기 적는다.
