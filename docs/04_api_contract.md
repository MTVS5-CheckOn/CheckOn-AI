...
| 🔴 **동기 (200)** | `/labels/suggest` | 🔴 **잡을 안 만든다** — 한 요청 = 학부모 **한 명** · **1콜**이라 그 자리에서 계산해 돌려준다(2026-08-22 판정 · §3.7). ⚠ 종전에는 **202 + 폴링** 이었고 그 전에는 **202 + Kafka** 였다 — **트리거가 바뀌면서**(8/21 · §3.7) 형태가 두 번 따라왔다. 🔴 **202 를 쓰면 워커·드레인·`WorkerKind`·마이그레이션 여덟 자리가 딸려 온다** — 그 비용이 이 판정의 근거다. **AI 호출 상한은 20초**이며, **BE read timeout은 20초보다 길게(권장 30초 이상)** 둔다. 상한 초과 응답은 **504 `TIMEOUT`**이다. |
...
🔴 **`Idempotency-Key` 가 없는 이유는 `kind` 마다 다르다(2026-08-25 · №97).**
· **`classification`** — 자연키(`inquiry_ref`) 행을 **갱신**한다 ⇒ 두 번 와도 같은 상태다.
  **재시도가 안전하다.**
...
🔴 **v1 응답 스키마 — A 판정 완료 · BE 합의 대기 (2026-08-24)**
⚠ 🔴 **「보류」가 아니다.** 「보류」는 «우리가 아직 안 정했다» 로 읽히는데, **우리는 정했다**
— 상대가 아직 안 봤을 뿐이다. 그 둘은 다르다.
· **A 판정**: 상태코드 의미론 ①~⑥ 과 제안 객체의 `label: {axis, value}` **중첩**(99 #215).
  중첩인 이유는 아래 예시 옆에 적었다 — 4축 값의 정의를 한 곳에 두려는 것이다.
· **대기 중인 것**: 🔴 **BE 합의 기록**뿐이다. 「확정」으로 바꾸려면 PR 설명에
  **누가·언제·어디서** 승인했는지 적는다(CLAUDE.md §2 승인 근거 규율).
🔴 **순서를 지킨다** — 04 «A 판정 · 합의 대기» → 시트 전달 → BE 회신 → **그때 「확정」**.
...
// Response **200 (동기)** — 학부모 360 화면에 '점선 칩'으로 표시 → 강사 확정(→ /confirmations) 전 초안에 미사용
{
  "data": {
    "suggestions": [{
      ...
      //    그 재사용이 깨지고 **4축 정의가 두 곳으로 갈린다.** 현재 구현은 중첩이며,
      //    v1 계약 확정은 BE 합의 대기다(99 #215).
      // ⚠ 🔴 `confirmations` 의 정정값(분류·태그)은 **평탄**하지만 **성질이 다르다** —
      //    그쪽은 **여러 축을 한 덩어리로** 확정하고(`{topic, sentiment, urgency}` ·
      //    `{area_tag, type_tag, item_format}`), 라벨은 **제안 하나가 축 하나**이고
      //    `suggestion_id` 도 제안마다 따로다. 라벨 확정(**v1 미구현**)의 정정값은
      //    `{axis, value}` **한 쌍**이 될 것이고 그건 이 `label` 안 내용과 **같은 모양**이다
      //    ⇒ 🔴 **두 모양이 섞이지 않는다**(8/24 판정 · #215).
...
② 🔴 이력이 **마스킹 문지기에 전부 걸려 0건**이 되면 `500 INTERNAL` 이다.
...
LLM 전송 상한 20초를 넘기면 **504 `TIMEOUT`**, 벤더가 가용하지 않으면
**503 `LLM_UPSTREAM_DOWN`**이다. 두 경로 모두 게이트웨이 전송은 **1회(재시도 0회)**다.
마스킹 전량 차단의 500 INTERNAL은 이 전송 실패들과 별도 사건이다.
  · 검사: `tests/ai/contract/test_labels_response_semantics.py::test_gateway_failures_use_the_canonical_http_mapping_without_retry`
  · 20초 배선 검사: `tests/ai/contract/test_labels_response_semantics.py::test_label_gateway_resolves_the_prompt_to_twenty_seconds_and_one_attempt`
...
⑥ 🔴 **`504 TIMEOUT`** — 🔴 **LLM 호출**이 콜당 상한(**20초** · `call_timeouts.yaml`)을
넘겨 **우리가 끊은 것**이다. ⚠ **서버 장애가 아니다.**
⚠ 🔴 **요청 전체 상한이 아니다(2026-08-24 실측 · 99 #220)** — 그 상한은 `LlmGateway.complete`
만 감싼다. 조립·마스킹·DB 가 느리면 **20초를 넘겨도 `504` 가 안 나고 `200` 이 늦게 온다**
(실측: 21.0초 걸린 요청이 **200**). 🔴 **최악 = (조립분) + 20초**이고 조립분은 실측
**2ms**(이력 10건 상한 · 로컬 · n=10)라 실질 **≈ 20초**다 — `/detect` 가 «45 + 15 = 60s» 로
적은 그 형식이다(§2.4). 🔴 **BE read timeout 이 최종 방어선**이다. 🔴 ①(빈 배열)·②(500)와 **또 다른 사건**이다:
①은 「제안이 없다」, ②는 「이력이 다 걸려 계산을 못 했다」, ⑥은 「시간 안에 못 끝냈다」.
⇒ 🔴 **재시도해도 같은 결과가 보장되지 않는다**(99 #203) — 재시도로 감추지 말고 강사에게
노출하는 편이 맞다. ⚠ 상세·본문은 안 싣는다(실측 8/24 — 이력 문면이 응답에 없다).
  · 검사: `tests/ai/contract/test_labels_response_semantics.py::test_exceeding_the_call_cap_is_a_504_not_a_500`
...
⚠ 🔴 **집계 `(축, 제안값, 확정값)` 은 확정 경로에서 남긴다** — `guardian_ref` **없이**. 개인에 대한 판단이 아니라 **우리 모델의 성적표**다. ✅ **구현**(2026-08-25 · `LABEL_CONFIRMATION_STAT`) — 제안 API 의 일이 아니다.
⚠ 🔴 **(2026-08-25 정정) 이 집계는 섀도 지표의 재료가 아니다.** 종전 문면이 `part_a/12_label_discovery.md` §6 착수조건 ③(섀도 지표)과 이어 놨는데 🔴 **실측으로 안 이어진다**(99 #250): 이 집계는 «**어느 축의 제안이 얼마나 뒤집히나**» 에 답하지만 «4축이 X% 를 못 담는다» 에는 **답하지 못한다** — `rejected` 는 «이 제안이 틀렸다» 이지 «이 축이 이 사람을 못 담는다» 가 아니고(`comm:data` 거절이 `comm:narrative` 를 배제하지 않는다), 🔴 **가장 강한 증거인 「제안 0건」은 확정이 안 와 아무것도 안 남는다.**
...
| `assignment_window` + `weekly_activity` (`enrolled_seconds=604800`) | `sha256:af17b16ba3517d7507c697940b8c43e76acb211c3468243a5c541d4f92c2c179` |
...
- **정본 계약:** 신규 `weekly_activity` 행의 `enrolled_seconds`는 **필수**다. 해당 주
  재원 구간의 초 단위 strict 정수이며 `1..604800`(7일)만 허용한다. 0초 행은 백엔드가
  보내지 않으며 문자열·실수·boolean도 **400 `INVALID_SCHEMA`**다. 이 값은 canonical
  해시에는 포함하지만 R3 판정에는 사용하지 않는다.
- **수신 구현의 legacy 관용:** 없음. 키 누락·`null`·범위 밖 값은 정본 계약과 동일하게
  **400 `INVALID_SCHEMA`**로 거부한다.
