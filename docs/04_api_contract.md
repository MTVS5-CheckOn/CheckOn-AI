...
### 3.7 `POST /v1/labels/suggest` — 라벨 제안 (🔴 **강사 요청**·202)

```json
// Request — 🔴 강사가 라벨 검토함을 열 때, 그 화면의 학부모 중
//           소통 이력 5건 이상 + 라벨 미설정인 대상만 실어 보낸다
{
...
🔴 **저장 정책 — 제안은 강사가 검토하는 동안만 보관한다.** counsel 초안과 **같은 축출 정책**을 따른다(⚠ 값을 여기 적지 않는다 — 운영값은 설정이 정본이다). 강사가 수락하면 `label_snapshot` 으로 확정되고 **제안 행은 버린다.** ⇒ 🔴 **새로 영속되는 개인 데이터가 없다.**
⚠ 다만 **집계 수는 남긴다** — `(축, 제안값, 확정값)` · 🔴 `guardian_ref` **없이**. 개인에 대한 판단이 아니라 **우리 모델의 성적표**이고, `part_a/12_label_discovery.md` §6 착수조건 ③(섀도 지표)이 그 수를 요구한다.
