...
🔴 **v1 응답 스키마 확정 보류 (2026-08-24)** — 상태코드 의미론은 아래 코드·검사와
일치하지만, 제안 객체의 `label: {axis, value}` 중첩 형태는 **BE 합의 기록이 없다**(99 #215).
현재 예시는 실제 응답을 기술할 뿐 **v1 확정 선언이 아니다.** 키를 확정하려면 PR 설명에
누가·언제·어디서 승인했는지 기록한 BE 합의가 선행돼야 한다.
