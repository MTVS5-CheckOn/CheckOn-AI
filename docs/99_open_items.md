...
| B-7 | evidence resolver 주입 시그니처 | evidence/resolver.py | ◐ **(7/28) A-5와 병합** — 조건 5개(fail-closed·권리 게이트·결정론·예산 불변·blind 무오염) 수용, A 초안 PR(`feat/evidence-resolver-v1`) 예정. `evidence/`가 현재 코드 0줄이라 회신이 아니라 A가 Protocol + Fake로 먼저 세운다. `GraphContextService`·`ContextPack` 실물 미의존(`part_b/11` §0) |
| B-8 | **VersionSet 양자 개정 2건 [양자]** | contracts/execution.py · 04 §2.2 · 06_erd AI_RUN | **ⓐ capability별 validator — ☐ 미결(7/22 등록):** A 실행에 B 버전 키 혼입 금지·B 실행에 B 키 필수를 `ExecutionContext` 조립 경계에서 강제할지. `execution.py`(양자 승인 파일) 변경이라 A·B 강제 수준 합의 필요(04 §2.2 크로스체킹 회신). **ⓑ GraphRAG 3필드 확장 — ◐ A 조건부 승인(7/28):** `content_graph_version`·`graph_index_version`·`retrieval_config_version` 추가, **기존 `graph_version`(교육과정 DAG · `d5283d0` 승인·구현 완료) 재사용 금지가 핵심**. 조건 ⓑ(`graph_index_version`↔`retrieval_config_version` 분리 근거) **B 회신 대기** · `meta.versions` 10 → 13키라 04 §2.2·06_erd 동시 개정. **`part_b/11` §11이 이 건을 B-8로 등록해 99 기존 B-8(validator)과 번호가 겹친다 — A는 한 행에 ⓐⓑ로 병합, B 문서 번호 정정은 B 판단** |
