...
| **R-8** `[구현 — 2026-08-19]` | 외부 자료 대조 | **생성 지문**과 외부 코퍼스(AI Hub 71857 지문 2,628건)의 **포함도**. 8자 shingle · `zlib.crc32` 표본 · 임계값 `verify_config.external_similarity_max`. R-6은 `previous_items`(같은 세트·같은 학생 최근 출제분)만 본다 — 축이 다르다(R-6=발문/세트내 · R-8=지문/외부). 🔴 **발문은 대조하지 않는다**(수능 발문은 정형이라 상시 발화한다) · 🔴 **T3 발췌도 대조하지 않는다**(버전 고정 만료 원문의 의도된 인용) | ✅ 코퍼스가 주입된 환경에서만 수행. **미주입은 통과가 아니라 `external_reference_checked=False`** 로 남는다 — 경로가 있는데 못 읽으면 예외(오설정≠미설정) |
