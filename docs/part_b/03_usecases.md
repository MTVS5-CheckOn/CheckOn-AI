...
1. 멱등 검사를 먼저 수행한 뒤, 새 요청의 `base_revision_no`가 최신 리비전과 다르면 **409 `REVISION_CONFLICT`**(`detail.reason=stale_base_revision`) — 최신본 재로드 후 새 멱등키로 재시도. 같은 멱등키+다른 바디의 **409 `IDEMPOTENCY_CONFLICT`**와 구분한다.
2. 문항당 진행 중 refine 1건만 — **새 멱등키**의 후속 요청은 409 `REVISION_CONFLICT`(`detail.reason=revision_in_progress`)로 즉시 거부한다. 같은 멱등키+같은 바디 재전송은 기존 처리 상태·결과를 200으로 재반환하고 중복 리비전을 만들지 않는다.
