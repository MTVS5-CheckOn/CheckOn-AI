# [체크온] curriculum_graph 사양서 v1 — 진단 입력 계약 · 5영역 그래프 · 약점 판정 · 결정론 규격
...
>
> **변경 이력**
> - v1.7 (2026-08-19): **57노드 전부에 2022 개정 성취기준 코드 근거를 붙였다 — `9.1`을 닫는다.** 🔴 **종전 상태가 근거의 순환이었다** — 언어 33노드는 `source_refs` 가 **아예 없었고**, 신규 4영역 24노드는 `area_specs.yaml`(우리가 쓴 출제 규격)만 가리켰다. **우리 문서를 우리 문서로 근거 대는 구조**였고, 그 노드들이 진단 판정의 축이다. ⓐ 정본(교육부 고시 제2022-33호 [별책 5])에서 고등 성취기준 **119종**을 뽑아 `curriculum_standards.yaml` 레지스트리로 만들고, ⓑ 57노드가 `curriculum-2022:<code>` 로 인용하며(고유 36종 사용), ⓒ `source_stage` 를 `official_public` 으로 올렸다 — enum에 값만 있고 **쓰인 적이 없던 자리**다. ⓓ 인용 코드의 실재·고등 범위·영역별 과목 정합을 CI가 대조한다. 🔴 **레지스트리에는 코드만 적는다** — 정본 라이선스가 공공누리로 **추정될 뿐 확인되지 않아**(05 §1.1.4 · W15), 사전 뜻풀이를 「참조만 하고 복제하지 않는다」로 정한 선례를 따랐다. 문면이 새어 들어오면 테스트가 막는다. ⚠ **교차 검증을 했다** — AI Hub 71857이 실제 고등 문항에서 관측한 성취기준 **17종이 전부** PDF 추출 레지스트리 안에 있었다(독립 자료 두 개가 일치). ⚠ **근거가 붙었다고 검수가 끝난 것이 아니다** — 「노드 ↔ 성취기준」 대응은 교과 판단이고 여기 적힌 것은 **초안 매핑**이다. `review_status` 는 그대로 `expert_review_pending` 이며, 달라진 것은 **검수자가 무엇을 검수해야 하는지가 명시됐다**는 점이다.
> - v1.6 (2026-08-11): 실파일을 `curriculum-five-area-v1`로 올리고 기존 언어 33노드는 유지한 채 독서·문학·화법과작문·매체에 각 6개 최소 노드를 추가했다. 신규 노드는 `area_specs.yaml` 출처 단계 ①의 v1 초안이며 전문가 검수 대기다. 근거가 없는 신규 선수 간선은 두지 않았다.
...
## 2. 그래프 5영역 — 공통 골격, 검수 수준과 엣지 근거는 다름

| 그래프 | area_tag | 구조 | 엣지 의미 | 예 |
| --- | --- | --- | --- | --- |
| 문법 DAG | `language` | 방향 비순환 그래프 | `requires` — 엄격한 선수 관계 | 음운 체계 → 음운 변동 → 표준 발음 |
| 독해 기능 초안 | `reading` | 6노드 · 신규 간선 없음 | `area_specs` 측정 축 | 정보 확인·추론·비판·보기 적용 |
| 문학 기능 초안 | `literature` | 6노드 · 신규 간선 없음 | `area_specs` + 만료 작품 풀 대조 | 표현·구성·화자·서술자·감상 |
| 화법과작문 기능 초안 | `speech_writing` | 6노드 · 신규 간선 없음 | `area_specs` 측정 축 | 말하기 전략·상호작용·계획·자료·고쳐쓰기 |
| 매체 기능 초안 | `media` | 6노드 · 신규 간선 없음 | `area_specs` 측정 축 | 정보·의도·신뢰성·구성·표현·수용자 |

신규 4영역은 **교과 정본이 아닌 최소 v1 초안**이다. `area_specs.yaml`만으로 확실한 선수 관계를 정할 수 없어 간선을 비웠고, 직접·셀 근거에 따른 `suspect`·`weak_confirmed` 산출까지만 연다. **중등·내신은 v1 범위 아님(7/15 확정)** — 그래프는 수능 고등 기준 단일, 중등 분기 금지.

**실파일 산출물:** `curriculum_graph.yaml`은 언어 33 + 신규 4영역 각 6 = 총 57노드다. 문학 6노드는 만료 작품 풀의 다섯 갈래(고전시가·현대시·고전산문·현대소설·수필)로 모두 출제 가능하다 — **`[2026-08-19 정정]` 종전에 「풀이 없어 넣지 않았다」고 적은 수필은 229편이 등재됐다.** 남은 미보유 갈래는 **극(희곡·시나리오)** 하나다. 상세 실측은 [`09_integration_proposals.md`](09_integration_proposals.md) 끝 절을 따른다.

**성취기준 근거:** 57노드 전부가 `source_refs` 에 `curriculum-2022:<code>` 를 달고 `source_stage: official_public` 을 기록한다. 인용 어휘의 정본은 `diagnosis/data/curriculum_standards.yaml`(고등 119종 · **코드만**)이며, 실재하지 않는 코드·고등 범위 밖 코드·영역과 어긋난 과목 인용은 CI가 막는다.
...
>
> ⇒ **파일럿 전까지 어떤 고정 컷도 판정 근거로 쓰지 않는다.** 셀 verdict는 강사에게 보여주는 힌트이며, **자동 경로로 흐르는 유일한 값은 `overall_low`**다(아래 분모 규칙이 그 노이즈를 완화한다). A 쪽 대칭 서술은 [`part_a/13`](../part_a/13_threshold_validation.md) **§5-4**(셀 판정의 통계적 성격)다 — 표본 희석을 논할 때 §5-2를 인용하지 않는다. §5-2의 "그룹당 8문항"은 **`unknown`(판단 보류)의 근거**이며 §5-2 본문의 🔴 근거 분리가 그 점을 명시한다. 컷의 분위 전환은 `part_a/13` §9 사다리와 같은 순서를 따른다 — 고정값으로 시작해 데이터가 쌓이면 분위로 옮긴다.

> ⚠ **③ 골든 코퍼스가 셀 공간을 대표하지 않는다 `[2026-08-04 실측]`.** 위 두 오차와 별개로, **판정 로직을 검증하는 데이터 자체가 얇다.**
>
> | 파일 | 이벤트 | 실질 `area × type` 셀 |
> | --- | ---: | ---: |
> | `evaluation/golden/diagnosis/diagnosis_cases.json` | 123 | **3** — `(language,concept)`·`(language,fact)`·`(reading,fact)` |
> | `tests/ai/unit/diagnosis/diagnosis_seed_cases.json` | 14 | **2** |
> | `part_a/examples/detect_demo_request.json` (참고) | 973 | **2** — `infer` 하나뿐 |
>
> **24셀 중 합집합 5셀이고 `critic`은 0건이다.** 따라서 ⓐ 셀이 늘었을 때 `unknown` 라우팅이 실제로 도는지 ⓑ `overall_low` 분모 규칙이 다양한 조합에서 성립하는지가 **골든으로 잡히지 않는다.** 합성 케이스로 단위 테스트는 가능하지만 **코퍼스 대표성은 별개 문제**이며, 셀 관련 수치는 낙관 쪽으로 편향돼 있다고 보아야 한다.
>
> A 쪽 픽스처에도 같은 문제가 있어 공동 안건으로 등록했다 — [`09`](09_integration_proposals.md) §3 W14 · 99 D ㊵ ㉠.
