...
>   분리한다. 상세 경계는 [`12`](12_suneung_format_alignment.md).
> - v1.3 (2026-07-27): **§9.1 GraphRAG 회귀(GR1~GR11) 신설** — 검색 0건·미승인 권리·라이선스 만료·hash 변조·coverage 결손·테넌트 교차·blind 누출·장애·정렬 재현성·수정 턴 재확인([`11`](11_graphrag_knowledge_layer.md) §10 P1-5). GR1~GR3·GR6·GR7은 **미탐 0건 CI 게이트**. §1 트리에 `golden/problems/graphrag/` 추가. B-M2-02·04·05 확정에 따라 §10 난이도 회귀에 요청 band 불일치·난이도 재생성 스위치·공통 예산 경계·재차 불일치, `t1_light_mode=false` 기준 전체 검증, 수동 목표 첫 성공 문항을 고정.
...
│  ├─ suneung_format/    # §4.1 수능형 합성 포맷·현 계약 간극
...
### 4.1 수능형 합성 포맷 (`suneung_format/`)

평가원 기출 원문·선지·도표를 저장소에 복제하지 않고 동일한 **구조만** 합성한다.

| 케이스 | 구조 | 기대 |
| --- | --- | --- |
| SF1 | 여러 줄 설명 자료 | 기존 `stem` JSON 왕복 |
| SF2 | 텍스트 표 | 구분자·줄바꿈 보존 |
| SF3 | ㉠~㉣ 예문 묶음 | 원문 기호 보존 |
| SF4 | 옛한글 문자열 | 유니코드 코드 포인트 보존 |
| SF5 | ㄱ·ㄴ 조합형 선지 | 선지 문자열 무손실 |
| SF6 | 자료 1개를 문항 N개가 공유 | `design_gap` 명시 · LLM 호출 0 |

SF1~SF5는 **표현력 회귀**일 뿐 교과 정답·난이도 검증이 아니다. SF6은 현
`GeneratedItem`에 공유 자료 식별자가 없다는 사실을 고정하며, [`12`](12_suneung_format_alignment.md)
FMT-1·FMT-2 합의 전 문자열 복제로 우회하지 않는다.
...
- taxonomy §2에서 확정한 경계 사례 7건을 시드로 과제명 60건에 대해 B 독립 라벨링(A와 상호 미공개 후 대조) — 불일치는 경계 사례집의 후속 증보 후보로 기록한다.
