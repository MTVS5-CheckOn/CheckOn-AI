...
PDF 요구사항은 정답률·문항수 두 카드에서 각각 3개월 막대를 그린다. 현재 입력과 결정론
조립기만으로는 월별 시계열을 만들 수 없으므로 `monthly_trend`도 미산출 축이어야 한다.
...
넷째 축 추가는 enum과 검증기를 함께 바꾸는 공용 계약 변경이다. 이미 대기 중인
`contracts/report.py` 양자 리뷰에 `monthly_trend` 편입 판단을 얹어 달라고 요청한다.
...
| **B-18** `[신규]` | **`ReportUnproducedMetric`에 월별 추이 축이 없어 PDF의 3개월 정답률·문항수 막대가 미산출로도 표현되지 않는다.** | 기존 `contracts/report.py` 양자 리뷰에서 `monthly_trend` enum 편입과 `validate_completeness` 4축 전환을 함께 판정. 승인 전 HTTP 키 노출 금지 | A+B | §2-31 |
