...
| 한계 | 구현 자리 | 잡을 만드는가 |
| --- | --- | --- |
| `area_tag ≠ language` 또는 `passage` 있음 | `application/workflow.py::_validate_execution()` | 🔴 **만든다** — 실행 중 거절이라 실패 잡이 남는다(99 #01 · 문 앞 이동 대기) |
| `type_tags`에 예약 태그 | `enqueue.py::reject_unsupported_type_tags()` | **안 만든다** — 잡 생성 이전이다 |

⚠ 두 줄의 차이가 99 #01의 전부다. 4xx가 실패 잡을 남기면 원장을 읽는 사람이 *"이 테넌트에
실패가 많다"* 로 오독한다. 처방은 「호출자 잘못은 문 앞에서 막아 잡을 만들지 않는다」이고,
예약 태그가 첫 사례이며 남은 자리는 조달 미구현 하나다.
...
⚠ **지금은 결함이 아니다** — 어휘 문항은 T2(독서) 지문 종속 유형이고(`05` §1) T2는 자료 조달
노드가 없어 400으로 막혀 있다. **막힌 문 뒤의 미배선**이라 도달 경로가 없다.

⚠ **그래서 등재한다.** T2를 여는 PR이 「생성 노드」만 붙이고 이 배선을 빼면, 어휘 문항이
**대조 없이** 나간다 — 그때는 R-1의 절반이 조용히 통과한다. **T2 개방 시 선행 항목**이다
(99 ㊦의 「없는 것을 상대로 짠 가드는 조용히 통과한다」와 반대 방향의 같은 실수 — 이쪽은
**있는 것을 안 쓰는** 형태다).
