...
현재 지원 경계는 둘이며 선후 관계를 섞지 않는다.

1. **자료 조달 게이트:** `language + passage 없음`과 `reading + PassageRequest`만 통과한다.
   `reading`에 생성 규격이 없거나, 다른 영역이거나, `language`에 passage가 있으면 400이다.
...
선행돼야 하고, T3는 LLM 생성 대상이 아니라 BE가 제공하는 라이선스 확인 원문 풀에
종속된다. 그래프 5영역 확장은 **자동 목표의 선행 조건**이며 이 요청 계약 작업과 독립이다.
...
없는 출제 액션**에만 붙인다. T3·T4·T5와 `PassageRequest` 없는 독서 요청이 그 대상이다.

| 우선순위 | 작업 | 의미 |
| --- | --- | --- |
| ✅ 완료 | `reading + PassageRequest` T2 본문 경로 | 지문 생성 뒤 같은 ContextPack 계열로 문항 생성 |
| P1 | `PassageRequest` 다영역화 | T4·T5 요청 계약과 영역별 생성 입력을 먼저 확정 |
| P1 | T3 라이선스 원문 풀 주입 계약 | LLM 원문 생성 없이 BE의 `license_ref`·버전이 있는 풀만 사용 |
...
| — | T3 문학 풀 초기 등재 규모·라이선스·주입 계약 | BE가 `license_ref`와 버전이 있는 원문 풀을 제공해야 하며 LLM 원문 생성은 금지 | BE+기획 | 05 §3 |
