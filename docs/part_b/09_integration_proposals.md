...
### 2-25. M2 백엔드 Kafka 명세 회신 — 화면 4스텝 배선 `[B 구현 · 2026-08-12]`

백엔드가 `codex/feature/problem-generation-kafka-recovery/37`(commit `49f46cb`)로 요청
Outbox·결과 컨슈머·미러링을 구현하고 명세를 보냈다(`PROBLEM_GENERATION_AI_TEAM_HANDOFF.md`).
회신 정본은 `docs/handoff/2026-08-12_m2_problem_generation_order_to_BE.md`이며, 여기에는
**B가 정한 판단과 남은 공용 문서 요청만** 적는다.

#### 2-25.1 백엔드 §7의 전제가 틀렸다 — `payload.result`에 본문이 없다

백엔드는 *"전체 `result`를 이벤트에 실으면 프론트 조회 API로 반환할 수 있다"* 를 전제로
1안·2안을 물었다. `ProblemSetResult`는 **세트 요약**이라 `item_id`·상태·수량만 있고
발문·선지·정답·해설·근거가 없다. 즉 **1안을 택해도 Step 3 화면은 안 그려진다.**

본문은 `ProblemItemStore`에 있었고 **노출 경로가 0건**이었다 — 계약도 저장도 있는데
읽는 자리가 없는 형태(#22가 등재한 그 부류)다.

**⇒ B 확정: 요약·참조는 이벤트, 본문은 REST.** `GET /v1/problems/{job_id}/items`를 신설했다.
근거는 ⓐ 이벤트 페이로드에 자유 텍스트를 싣지 않는 규약(`08_kafka_events.md` §5) ⓑ 어느
안을 택해도 Step 3에는 REST가 필요하므로 두 경로를 만들 이유가 없다 ⓒ 문항 10개 본문이면
메시지가 수십 KB이고 보존 기간 동안 남는다.

⚠ **전체 결과를 이벤트에 싣는 안으로 가려면 `08_kafka_events.md` §4 개정이 필요하다** —
A 소유 공용 문서라 B가 단독으로 뒤집지 않는다. 백엔드가 REST 클라이언트를 못 만든다고
회신하면 그때 A 승인 안건으로 올린다.

#### 2-25.2 이번에 구현한 것

| # | 변경 | 자리 |
| --- | --- | --- |
| ① | `GET /v1/problems/{job_id}/items` 신설 — 문항 본문·상태 4종 집계 | `api/routers/problem.py` |
| ② | `POST /v1/diagnosis` 신설 — Step 1 `area×type` 그리드 | `api/routers/diagnosis.py` |
| ③ | 진단 설정 로더 + `diagnosis_config.yaml`(`b-defaults-v1`) | `diagnosis/config.py` |
| ④ | `GET /v1/problems/{job_id}`의 프로세스 캐시 의존 제거 | `api/routers/problem.py` |
| ⑤ | Kafka 이벤트 순수 변환 계층 + fixture 4종 | `problem_generation/infrastructure/kafka_contract.py` |

🔴 **③이 없어서 진단 임계값을 만드는 자리가 테스트에만 있었다.** 프로덕션 호출자가
`DiagnosisConfig(...)`를 새로 적어야 했고, 그건 *"값이 바뀌면 코드 diff가 생기면 위치가
틀린 것"*(03 §1)의 그 위치다. `cell_min_items`는 A threshold 시트 R6가 정본이라 값을
복제하되 드리프트를 `tests/ai/contract/test_diagnosis_config.py`가 CI에서 잡는다 —
capability 경계 때문에 `diagnosis/`가 `detection/`을 import할 수 없어서다.

🔴 **④는 완전한 해소가 아니다.** 잡 원장이 정본이 됐지만 요청·결과 저장소 기본이
인메모리라 **재기동 후에는 여전히 결과를 못 읽는다.** 백엔드 §14의 "다중 인스턴스 지원"에
정직하게 답하려면 `ProblemRequestStore`·`ProblemResultStore`의 PG 구현이 남는다 —
`PgProblemItemStore`만 있고 나머지 둘은 없다. **미해소 등재 대상**이다.

⑤는 `CLAUDE.md` §3의 선 안이다 — `aiokafka` 의존도 브로커 코드도 없고, envelope ↔ 내부
계약의 매핑만 한다. **컨슈머·프로듀서·outbox는 여전히 미구현**이고, terminal outbox는
잡 저장소 PG 전환이 선행이다(인메모리에는 트랜잭션이 없다).

#### 2-25.3 그리드 표시 규약 재확인 — §2-21과 어긋나지 않게

백엔드 화면 설명이 「5×4 그리드」였는데 §2-21.1의 확정은 **측정 5 · 표시 4**다
(`language`+`media`가 화면에서 한 행). API는 **측정 축 5개를 그대로** 내보내고 병합은
클라이언트가 한다 — 회신 문서 §4-4에 표로 적었다. 유형 열 한글 라벨을 싣지 않는 것도
§2-21.2 확정 그대로다.

**신규 판정 1건:** 그리드의 빈 칸을 `verdict: null` · `acc: null` · `n: 0`(`no_data`)으로
내보내고 `unknown`(1건 이상·최소 표본 미달)과 **가르지 않는 표현을 쓰지 않는다.** 표본 0에
`acc: 0.0`을 채우면 **정답률 0%로 읽히고**, 두 상태는 강사의 다음 행동이 다르다
(*"출제해서 재보자"* vs *"조금 더 모으자"*). `WeaknessCell`은 `acc`가 필수 float이므로 이
표현은 계약이 아니라 **라우터의 표시용 투영**에 둔다 — 계약을 화면 사정으로 오염시키지
않는다.

#### 2-25.4 공용 문서 요청 `[A 반영 대기]`

| 대상 | 요청 | 사유 |
| --- | --- | --- |
| `docs/04_api_contract.md` | `GET /v1/problems/{job_id}/items`·`POST /v1/diagnosis` 2건 편입 | 계약 정본에 B 엔드포인트가 없다(§2-19와 같은 축) |
| `docs/08_kafka_events.md` §4 | `worker_job.*` payload에 `operation`·`set_id`·`result_status`가 실린다는 것 명시 | 백엔드가 상태를 읽어야 미러링이 선다. **본문 미포함 규약은 그대로다** |
| `docs/02_ownership.md` §5 | 트리에 `api/routers/diagnosis.py` [염준영] 행 추가 | 라우터 신설 |
| `docs/99_open_items.md` | 아래 2건 색인 등재 | §4-6 규약 — 미해소는 소유와 무관하게 99에 한 줄 |

**99 등재 요청 2건** (본문은 이 절이 갖는다):

1. **PG 요청·결과 저장소 PG 구현 부재** — 재기동·다중 인스턴스에서 결과 조회 불가.
   `PgProblemItemStore`만 있고 `ProblemRequestStore`·`ProblemResultStore`는 인메모리뿐이다.
2. **Kafka 컨슈머·프로듀서·terminal outbox 미구현** — 백엔드는 발행 준비가 끝났고 AI 쪽
   소비자가 없다. 잡 저장소 PG 전환이 선행 조건이다.

⚠ `api/app.py`는 양자 승인 파일이다 — 라우터 등록 2줄만 바꿨고 `problem_router` 등록
선례와 같은 형태·같은 주석이다(**A 리뷰 요청**).

---

### 2-26. AI는 HTTP만 — Kafka는 별도 adapter로 확정 `[BE 확정 수용 · 2026-08-12]`

백엔드가 §2-25 회신을 받고 구조를 다시 잡았다(`PROBLEM_STUDIO_AI_TEAM_HANDOFF.md` §2):
**AI 서버는 Kafka consumer·producer를 구현하지 않고 HTTP API만 제공하며, 별도
Kafka-HTTP adapter 서버가 요청 소비·AI HTTP 호출·폴링·결과 정규화·이벤트 발행을 전담한다.**

#### 2-26.1 우리에게 좋은 확정이다 — 다만 규약 하나의 소유가 옮겨 갔다

Kafka 실연동(#25의 미해소 2번)이 **우리 범위에서 빠졌다.** terminal outbox·컨슈머·프로듀서를
만들 이유가 사라졌고, 잡 저장소 PG 전환의 선행 조건도 그쪽이 아니라 **결과 조회 영속성**만
남는다.

🔴 **그 대신 "이벤트에 본문을 싣지 않는다"(`08_kafka_events.md` §5)가 우리 규약이 아니게
됐다.** 그 규약은 **AI가 발행하는 이벤트**에 대한 것이고, 이제 이벤트를 만드는 주체는
adapter다. 백엔드가 정규화 이벤트에 문항 본문을 실을지는 **BE 결정**이며, 우리는 판단
재료(크기 실측·개인정보 경로 없음)만 제공했다. ⚠ **이걸 "우리 규약이니 따르라"로 밀면
소유 밖을 정하는 것이다.**

#### 2-26.2 백엔드 검증(승인 보류)에서 우리가 정정한 사실 4건

회신 정본은 `docs/handoff/2026-08-12_m2_http_contract_reply_to_BE.md`다. 합의 초안에 들어간
값 넷이 실측과 달랐다 — **틀린 숫자가 계약 문서에 남으면 adapter가 그 숫자대로 만들어진다.**

| 초안 | 실측 |
| --- | --- |
| *"OpenAPI를 유일한 정본으로"* | 🔴 우리 OpenAPI에는 **요청·응답 스키마가 0개**다(전 엔드포인트가 `Request`를 직접 읽고 `dict`를 돌려준다). 지금 정본으로 선언하면 **빈 계약이 잠긴다** |
| 멱등 보존 7일 | **30일**(`DEFAULT_TTL`) |
| `403 TENANT_MISMATCH` | 우리는 403을 내지 않는다 — 전부 **404**(존재 은닉) |
| *"v1은 language-only(AI 제약)"* | **5영역 전부 출제 가능**하다. 자료 명세가 함께 와야 할 뿐이고, language-only는 **프론트가 자료 입력 화면이 없다는 제품 사정**이다 |

🔴 **네 번째가 특히 위험했다** — 지적 10번이 *"진단은 media를 보여주는데 출제는 안 된다"* 를
설계 충돌로 다뤘는데, 사유를 *"AI 미지원"* 으로 적으면 나중에 자료 입력 화면이 생겨도
**아무도 그 행을 다시 열지 않는다.**
...
| # | 변경 | 사유 |
| --- | --- | --- |
| ① | HTTP fixture **17종** + 생성·대조 테스트 | adapter의 유일한 실행 가능 계약. **실제 앱을 돌려** 얻고, 응답이 바뀌면 CI가 먼저 죽는다 |
...
⚠ **HTTP 인증(현재 없음)** 은 미해소로 세지 않는다 — 7/15 BE-3이 *"내부망 전용 호출"* 로
확정한 전제 위에 서 있다. adapter가 망 밖에서 붙는 구조로 가면 그때 안건이 된다.
...
### 2-29. 리포트·브리핑 공통 결정론 텍스트 게이트의 중립 경계 `[A 판정 · 2026-08-24 · ⓑ 주입 경계 유지 · 계기 = 소비 축 셋째]`
...
**A 판정(2026-08-24): ⓑ 주입 경계를 유지한다.** `gates/`는 현재 `.gitkeep` 하나뿐이라
공통 구현 이동은 승격이 아니라 첫 입주이고, `contracts/gates.Gate` Protocol의
`check(payload, seq) -> contracts.gates.GateResult` 모양은
`check_brief_gate(text, allowed_numbers, max_length=...) -> briefing_gate.GateResult`와 다르다.
또한 `GateName` 열한 값 중 이 결정론 텍스트 검사를 그대로 가리키는 값이 없어서 새 값을
추가하려면 양자 파일 `contracts/gates.py`를 바꿔야 한다. 현행 호출부 8자리와 같은 모양에
걸린 다른 안건 둘까지 한꺼번에 옮길 근거도 아직 없다. 따라서 `check_brief_gate`의 **소비 축이
셋째가 되는 회차**를 공통화 계기로 삼고, 그때 A가 호출부 전수 가드를 세운다.

~~위 ⓑ 판정은 리포트 프로덕션 조립 주체가 별도로 존재한다는 전제를 뒀다.~~ **A 판정 정정
(2026-08-25 · ⓒ 채택):** 테스트가 게이트를 주입하는 것은 프로덕션 배선의 증거가 아니다.
`ReportNarrator`는 `DeterministicTextGate`를 필수로 받는데 실제 조립 주체가 없었으므로,
`report/provider.py`가 `check_brief_gate` 이름 하나를 직접 import하는 경계 예외를 명시한다.
예외 자리는 프로덕션 기준 정확히 한 곳인지 CI가 세며, 둘째가 생겨 소비 축이 셋째가 되면
목록을 늘리지 않고 ⓐ(`gates/` 승격)로 간다. 승격 전에는 토큰·기호·숫자 EXACT·길이의 일반
검사와 `buffer_lexicon` 금칙어인 브리핑 고유 검사를 먼저 갈라, 고유 규약을 공용으로 통째
옮기지 않는다.
...
### 2-31. 월별 추이 미산출 축의 공용 계약 편입 `[B 제안 · 2026-08-24 · A 리뷰 대기]`

`ItemCountsPayload`는 `scored_items`·`judged_items`라는 단일 집계값만 제공하지만, 학부모
PDF 요구사항은 정답률·문항수 두 카드에서 각각 3개월 막대를 그린다.

~~`DiagnosisInput.period`가 단일 기간이고 `ItemCountsPayload`도 단일 집계값뿐이므로 현재
입력과 결정론 조립기만으로는 여러 달의 월별 시계열을 만들 수 없다.~~ **오진 정정
(2026-08-24 · 저장소 실측):** `Period`는 `from_date`·`to_date` 양 끝을 받는 임의 구간이며
"한 달" 계약이 아니다. `DiagnosisEvent.occurred_at: datetime`은 필수 필드이고,
`detection/features.py:152`는 이미 이벤트 `occurred_at`을 월요일 기준 주차로 버킷한다.
따라서 같은 이벤트를 월별로 버킷하면 **계약 확장 없이 월별 추이를 산출할 수 있다.**
PR #385 본문의 같은 진단도 이 정정으로 대체한다.

그러나 공용 `ReportUnproducedMetric`에는 전국 백분위·주차별 정답률/개입·최근 6주 기준선
세 축만 있다. 그 셋은 응답에서 `not_produced`로 정직하게 비지만 월별 추이만 계약 어휘가
없어 조용히 사라진다. 라우터가 별도 문자열을 내보내면 `ReportStudioData.unproduced`와 답이
둘로 갈리므로, PR #385에서는 HTTP 키를 만들지 않고 이 안건으로 옮긴다.

**B 단독으로 고칠 수 없다.** `contracts/report.py`는 양자 승인 파일이고,
`ReportStudioData.validate_completeness`가 현재 enum 세 축의 순서와 완전성을 전부 강제한다.
넷째 축 추가는 enum과 검증기를 함께 바꾸는 공용 계약 변경이다. 다만 B-18의 성격은
"못 만든다"가 아니라 **"만들 수 있는데 `ReportUnproducedMetric`이 아직 미산출로 선언한다"**로
바뀌었다. 월별 추이를 산출 축으로 전환하면서 기존 미산출 세 축을 함께 정리하는 일은 여전히
양자 계약 리뷰 대상이다.

### 2-32. 문제 생성 프롬프트 redaction 배선 재측정 `[B 실측 · 2026-08-24 · 모순 잔여]`

**종전 후보 셋 중 남아 있던 ③도 성립하지 않았다.** T1 실측 기준 SHA `7301915`는
2026-08-21 커밋이므로 8/6의 redaction 조정 뒤다. 따라서 ② 「당시에는 8/6 조정 전이라
통과했다」는 시간축부터 맞지 않는다. 같은 SHA의 워크플로를 다시 읽으면 T1 `language`는
지문·자료 생성 분기를 타지 않고 `ProblemGenerationWorkflow`가
`ProblemGenerator.generate()`를 호출한다. 재시도는 `retry_context`만 바꾸며 호출 대상은
계속 `pg.items.v1`이고, 같은 SHA의 registry가 이를 `problem_generation/items.txt`에 연결한다.
따라서 §1.1 T1의 「생성 시도 2」에도 **`items.txt`가 실렸다.** ③ 「그 호출에는
`items.txt`가 없었다」는 반증됐다.

실 LLM은 호출하지 않았다. 현재 develop `6eb88f1`에서 여섯 공개 애플리케이션 호출을 기존
단위 테스트 입력으로 실행하고, 각 모듈의 전송 직전 `redact()` seam에서 **실제 렌더 원문과
`findings`·`uncertain`을 함께 기록**했다. 이어 실제 `LlmGateway`와 `FakeProvider`까지 도달시켜
단순 `redact()` 직접 호출이 아니라 배선을 통과했음을 확인했다. gateway는
`gateway.py:214`에서 조건 없이 `trace_masking_hook.mask()`를 한 번 더 호출한다.

| prompt_id | 실제 전송 경로 실측 | `findings` | `uncertain` |
| --- | --- | --- | --- |
| `pg.passage.v1` | 렌더 1,795자 · 원문과 masked 동일 | 없음 | `false` |
| `pg.source_material.v1` | 렌더 2,349자 · 영역 규격의 「다음은 학생의…」에서 `다음은`이 이름 토큰으로 바뀜 | `이름` 1건 | `false` |
| `pg.items.v1` | 렌더 6,119자 · `items.txt:30`의 「않고 학생」에서 `않고`가 `⟪이름1⟫`로 바뀜 | `이름` 1건 | `false` |
| `pg.cross_solve.v1` | 렌더 1,218자 · 원문과 masked 동일 | 없음 | `false` |
| `pg.misconception_check.v1` | 렌더 1,507자 · 원문과 masked 동일 | 없음 | `false` |
| `pg.refine.v1` | 렌더 6,741자 · `evidence_pack_id` UUID 앞 8자리가 연락처 토큰으로 바뀜 | `연락처` 1건 | `false` |

`items.txt`의 기전은 확정됐다. `redaction_patterns.yaml:101`의
`([가-힣]{2,3})(\s?(…|학생(?!들)|…))`에서 group 1이 `않고`가 되어
`Finding(type="이름", token="⟪이름1⟫")`이 생긴다. 이 문장은 `1fb9208`(2026-08-07,
「인명 예시로 인한 교차 풀이 차단 방지」)에서 들어왔다. **예전 인명 문제를 막으려고 넣은
문장이 이제 자기 자신을 이름으로 잡는다.** 다만 소비 코드는 `findings` 비어 있음이 아니라
`uncertain`만 전송 중단 조건으로 삼는다. 이 건은 `uncertain=false`라 토큰으로 바뀐 뒤
gateway와 provider까지 간다.

따라서 결과 갈래는 ⓑ다. `items.txt`가 있었는데도 전송 경로가 계속됐으므로 **모순은
해소되지 않았다.** §1.1의 당시 실측 기록이 무엇을 `500`으로 센 것인지, 당시 요청·응답 경로가
현재 재현과 어디서 달랐는지 근거가 더 필요하다. 원인을 모르는 채 프롬프트나 A 소유 redaction
규칙을 고치면 재현 근거가 사라지므로 이번 회차에서는 고치지 않는다.

A가 제외 목록에 쓰기로 한 문면은 다음과 같다.

> `"problem_generation/": "🔴 B 가 원문 템플릿에서 1건 검출(items.txt:30) · 렌더 경로 미확인(2026-08-24) — 걷는 계기는 B 재측정"`

이번 실측으로 「렌더 경로 미확인」은 해소됐지만 **제외를 걷는 조건은 충족하지 않았다.** 오히려
실제 전송 원문에서 `items`·`source_material`·`refine` 세 경로의 마스킹을 확인했다. A 소유
제외 목록은 B가 수정하지 않으며, A가 문면을 현행 실측으로 갱신하고 모순 판정 전까지 제외를
유지해 달라고 요청한다.

재발 방지 가드는 A의 `test_prompt_strings_pass_the_masking_gate.py`와 같은 두 층이어야 한다.
① registry의 원문 템플릿 전수 순회로 정적 문면 회귀를 잡고, ② 공개 애플리케이션 호출을
실제 `LlmGateway`까지 보내 **전송 경로가 만든 렌더 프롬프트**의 `findings`와 `uncertain`을
동시에 단언한다. 원문 순회만 두면 영역 규격에서 조립된 「다음은 학생의」와 UUID가 들어간
refine 오탐을 보지 못한다. 이 가드는 A 소유 masking 경계이므로 여기서는 제안만 남긴다.

### 2-33. 리포트 생성의 검사 전용 주입으로 가려진 프로덕션 500 `[해소 · 2026-08-25 · 배선 완료]`

`POST /v1/reports`는 프로덕션에서 `ReportNarrator`가 한 번도 주입되지 않아
`require_report_narrator()`의 `RuntimeError`로 500이 났다. 반면 라우터·문장화 검사는
`set_report_narrator()`로 대역을 직접 꽂아 전부 green이었다. 프로덕션에서 비어 있는 자리가
red도 skip도 되지 않은 채 검사에서만 채워진, A가 이름 붙인 **「검사에서만 꽂히는 이음매」**다.

이번 배선으로 500 증상은 사라진다. 그러나 **사라진 것과 고친 것은 다르다.** provider를
신설하고 앱 startup이 주입 없이도 narrator를 조립하는지 검사하며, 이미 주입된 narrator는
덮지 않는다는 조건까지 고정해야 원인이 고쳐진다. 조용한 Fake 폴백 금지 설계는 유지한다.
fake를 선택한 경우에는 경고 로그와 `fake-report` provider 표기로 명시적으로 구분한다.

이 저장소에서 같은 형태는 세 번째다. `time_series`가 생산 조립에 배선되지 않았던 자리,
`set_report_narrator`가 테스트에서만 호출되던 자리, 그리고 이번 프로덕션 startup 미배선이다.
...
### 2-35. 리포트 `fact`·`chart_analysis` 0/6의 0콜 산술 `[B 실측 · 2026-08-26 · 재측정 대기]`

#450의 3차 실측은 리포트 6회·외부 68콜이었다. `greeting`은 6콜 모두 1회 성공했고,
`suggestion`은 성공 콜 4/8, `closing`은 성공 콜 2/18이었다. 반면 `fact`와
`chart_analysis`는 각각 18콜이 모두 `parse_fail`이어서 섹션 6/6이 3회 소진됐다.
재생성 38회와 `llm_unavailable`·`redaction_blocked` 0건은 #438의 재시도가 실제로
돌았음을 보이지만, 왜 두 블록만 빈 content였는지는 가르지 못했다.

검토한 가설은 `report/narration.py`의 공통 `max_tokens=256`이 부족하다는 것이다. 다만
다음 반증 셋을 함께 둔다. ① `_MAX_LENGTHS`의 160·360·480·320·160자는 산출 요구량이
아니라 생성 뒤 게이트가 넘으면 거부하는 상한이다. 실패한 두 블록의 상한이 큰 것은
상관일 뿐 인과가 아니다. ② A의 99 #49 실측에서 저쪽 문장화 산출은 14~32토큰이어서
256에는 최소 8배 여유가 있었다. ③ 같은 자리에서 reasoning이 예산을 쓴다는 선행 해석은
`reasoning_tokens=0` 실측으로 반박됐고, 0자의 원인은 게이트 소진이었다. 따라서
「빈 content = 토큰 부족」을 전제로 두지 않는다.

#### 구조화 응답 전체 길이 산술

다섯 프롬프트는 모두 `ReportNarrationDraft(content, numbers_used)`라는 같은 스키마의 JSON
객체 하나를 요구한다. `tests/ai/unit/report/test_narration.py`가 green으로 고정한 다섯
대표 응답을 compact JSON으로 직렬화해 **본문뿐 아니라 필드명·따옴표·배열·숫자까지 전부**
센 결과는 다음과 같다.
빈 content와 빈 배열만 둔 고정 구조 오버헤드는 32자다.

| prompt_id | content 문자 | 전체 JSON 문자 | UTF-8 바이트 | 게이트 문자 상한 |
| --- | ---: | ---: | ---: | ---: |
| `report.greeting.v1` | 18 | 50 | 78 | 160 |
| `report.fact.v1` | 21 | 55 | 85 | 360 |
| `report.chart_analysis.v1` | 18 | 50 | 78 | 480 |
| `report.suggestion.v1` | 21 | 53 | 85 | 320 |
| `report.closing.v1` | 21 | 53 | 87 | 160 |

저장소에는 `gpt-5.6-luna`의 tokenizer가 없으므로 정확한 토큰 수라고 쓰지 않는다. 다만
OpenAI 계열의 byte fallback을 가정한 보수적 상계에서도 대표 응답은 78~87토큰 이하라
256의 2.9배 이상 여유가 있다. 실제 BPE는 여러 바이트를 한 토큰으로 묶으므로 보통 이보다
작고, A의 14~32토큰 실측과도 방향이 같다. 게이트 상한까지 한글로 꽉 채운 가상 응답은
256을 넘을 수 있지만 프롬프트 어느 곳도 상한만큼 쓰라고 요구하지 않는다.

`suggestion`도 경계에 걸린 모양이 아니다. 대표 구조화 응답은 53자·85바이트로 다른 네
블록과 같은 크기이고, 스키마와 256 예산도 같다. 부분 성공 4/8은 변동을 보여줄 뿐 토큰
경계의 증거가 아니다. `classify`는 같은 256 예산으로 더 많은 필드를 가진 고정 JSON
114자·114바이트를 요구한다. `briefing`은 자유 문장이고 토큰 상한을 두지 않는다. 리포트는
본문 의미는 briefing과 닮았지만 **전송 형상은 짧은 JSON 하나인 classify에 더 가깝다.**

따라서 0콜 산술에서 「`fact`·`chart_analysis`만 훨씬 긴 산출을 요구한다」는 필요조건은
성립하지 않는다. **예산 가설은 현재 근거로 서지 않는다.** 다만 이것은 원인 확정이 아니라
다음 실측에서 `finish_reason`을 볼 우선순위를 낮추는 판정이다.

#### `parse_fail`이 뭉쳤던 두 경로와 다음 갈림

#450 당시 `openai_compat.py`의 빈·공백 content와 `llm/structured.py`의 JSON 파손은 모두
`ParseFailed`로 수렴해 같은 이름만으로는 구분할 수 없었다. #459(`d6b0ffc`) 뒤에는 전자는
`LLMResult(outcome=parse_fail, finish_reason=...)`, 후자는 구조화 파서의 `ParseFailed`로
갈라진다. 하네스도 원문 없이 콜 순번·벽시계 앞/중/뒤·finish_reason 수집 상태와 값만
남기도록 보강했다.

다음 재측정의 판정은 닫혀 있다. `vendor_provided/length`면 예산 가설이 다시 서고,
`vendor_provided/stop`인데 content가 비면 다른 원인이다. `vendor_not_provided`는 벤더가
필드를 안 준 것이고 `uncollected`는 대역·측정층이 안 읽은 것이므로 둘을 같은 「값 없음」으로
접지 않는다. 공개 HTTP 경로는 한 리포트에서 다섯 블록을 각 최대 3회 부르므로 **첫 갈림은
최대 15콜**이면 보인다. 반복성을 확인하려면 3회, **최대 45콜**이 필요하다. 실 LLM은 별도
승인 뒤에만 실행한다.

`runtime/console.py:349-350`과 `:368`은 아직 finish_reason을 「파이프라인에 없음·미수집」으로
표시한다. #459 뒤 사실과 어긋나지만 A 소유 파일이라 B는 건드리지 않았다. A가
finish_reason 줄만 실제 관측값으로 바꾸고, 열지 않은 `reasoning_tokens` 미수집 줄은 유지해
달라고 요청한다.
...
| **B-22** `[0콜 산술 · 2026-08-26 · 재측정 대기]` | **`fact`·`chart_analysis` 0/6의 토큰 부족 가설은 대표 구조화 응답 전체가 50~55자·78~87바이트이고 다섯 블록이 같은 스키마·256 예산을 써 지지되지 않는다.** #450 당시 빈 content와 JSON 파손은 `ParseFailed` 한 이름에 뭉쳤다 | #459의 finish_reason과 보강한 콜 순번·벽시계 축으로 재측정한다. 첫 갈림은 최대 15콜, 반복 확인은 최대 45콜. `length`면 예산, `stop`인데 빈 content면 다른 원인으로 판정한다 | B(+A 콘솔 표기) | §2-35 |
...
| **B-18** `[오진 정정 · 2026-08-24]` | **월별 추이는 이벤트 시각을 버킷해 계약 확장 없이 산출할 수 있지만, `ReportUnproducedMetric`은 아직 이를 미산출로 선언한다.** | 월별 추이를 산출 축으로 전환하고 기존 미산출 세 축을 정리하는 양자 계약 리뷰가 필요하다. 승인 전 별도 HTTP 키 노출 금지 | A+B | §2-31 |
| **B-19** `[실측 · 2026-08-24 · 모순 잔여]` | **T1 생성 시도는 `items.txt`를 실제로 싣는다. `않고 학생`은 이름 1건으로 마스킹되지만 `uncertain=false`라 gateway 전송이 계속된다.** 여섯 렌더 경로 전수에서 `source_material`의 「다음은 학생의」와 `refine` UUID도 각각 이름·연락처로 잡혔다 | 프롬프트·redaction 무수정. A가 두 층 가드(원문 전수 + gateway 실제 렌더)를 세우고, §1.1 당시 500의 정확한 경로를 재대조할 때까지 제외 유지 | A+B | §2-32 |
...
| C-15 `[해소 — 2026-08-19]` | 외부 표절·유사도 코퍼스 | **AI Hub 71857 지문 2,628건으로 닫았다.** `EXTERNAL_CORPUS_ROOT` 주입 · 저장소 반입 없음 · R-8 구현(06 §1). ⚠ **injection 코퍼스는 별개로 남는다**(05 §8.2) — 이 자료는 표절 축만 닫는다. ⚠ 상용 전환 시 W18이 되살아나며 그때 이 항목도 다시 열린다 | B+기획 | 05·08 |
