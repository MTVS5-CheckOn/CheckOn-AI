...
> **지위:** member-B(염준영) 공식 제안 목록 v1. **본 문서는 아무것도 확정하지 않는다** — A 소유 문서·공용 계약·정책·ERD 변경은 여기 기록된 요청/제안이 오너 승인(`docs/02_ownership.md` 절차)을 거쳐야 반영된다. B는 `docs/part_b/` 외 파일을 직접 수정하지 않았다.
>
> **변경 이력**
...
| (7/15 후속) contracts 공용 5파일 | ✅ A가 구현 완료(`feat/contracts-base` — taxonomy·execution·llm·gates·evaluation + 테스트 74종) | B 회신 §1-5 · 확장 제안 §2-3·§2-9 |
...
### 2-3. 양자 승인 확장 5건 — 공용 enum·계약 확장 (contracts 구현으로 대상 코드 구체화) `(C-07·C-09)`

| 대상 (코드 + ERD) | 변경 | 절차 |
| --- | --- | --- |
| `contracts/execution.py` `Capability` | `diagnosis`·`problem_generation` 추가 — 현 enum은 A 3종뿐(docstring이 "B 테이블 증분 시 함께 확장" 명시) | 양자 승인 — B 워커 구현의 선결 |
| `contracts/gates.py` `GateName` + `GATE_RESULT.gate_name` | `RuleValidation \| BlindCrossSolve \| ReleaseDecision` 추가 | 양자 승인 |
| `contracts/gates.py` `OwnerKind` + `GATE_RESULT.owner_kind` | `problem_set` 추가 | 양자 승인 |
| `contracts/gates.py` `BlockedReason` | `answer_integrity`·`banned_topic`·`prompt_injection` 추가([`07`](07_refine_policy.md) §3) | 양자 승인 + error_codes §2.2 동시 개정 |
| `EVIDENCE_ITEM.owner_kind` | `problem_item` 추가 | `evidence/models.py` 구현 시 양자 승인(현재 미구현 — ERD 제안 선반영) |

**PR용 코드 시안** (승인 시 그대로 반영 가능 — enum 동결 테스트 74종 중 해당 frozen 검사도 동시 개정):
...
| `contracts/diagnosis.py`·`problem_generation.py` 초안 | — |
...
### 2-9. `contracts/execution.py` `VersionSet` B 확장 제안 `[신규 — feat/contracts-base 후속]`

`VersionSet`은 6종 고정(`extra="forbid"`)이라 B 실행의 재현 키가 실릴 자리가 없다. `threshold_version`(detection 전용 nullable)과 같은 선례로 **B 전용 nullable 필드 확장**을 제안:
...
- 양자 승인 + `04_api_contract §2.2`·`06_erd AI_RUN` 동시 개정 대상. Capability 확장(§2-3)과 같은 PR 권장.

**PR용 코드 시안** (`threshold_version` docstring 패턴 준용 — `RunMetadata`·`to_run_metadata`·AI_RUN 컬럼·`test_run_metadata_matches_ai_run_columns` 동시 확장):
...
| B-2 | 공용 계약 리뷰 — 공용 5파일은 구현 완료(§0), **잔여: execution.py 신규 필드 승인 확정(§1-5) + Capability·VersionSet 확장(§2-3·§2-9) + B 소유 2파일(diagnosis·problem_generation) 초안** | §1-5 회신 기준 | A+B | 02 §5 |
