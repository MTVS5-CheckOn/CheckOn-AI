...
>
> **변경 이력**
> - v1.3 (2026-08-04): 트랙 5종 재정의(`05` §1)를 반영했다 — §4의 GraphRAG 관계 서술을 대조 트랙(T1 문법·T2 어휘) 기준으로 정정하고, §7.3 제목을 "대조 트랙 자료"로 바꿨다. `grammar_rule` 대응표 신설 항목은 **어문 규범 조항 번호 직접 사용**으로 대체했다(항별연혁정보가 개정 이력을 제공한다).
...
| **C3** | ✅ **해소(2026-07-27) — `KEEP-1`~`KEEP-9`로 설계 확정.** 첫 검증본을 **`ITEM_CANDIDATE`**(신규 8번째 테이블, unique `(tenant_id, set_id, slot_index, attempt_no)`)에 불변 스냅숏으로 저장하고 state에는 **포인터만** 둔다(§2.4 "본문 미복제" 준수). 재생성본이 게이트 ①·② 실패 또는 `verification_unavailable`이면 첫 검증본을 최종본으로 복귀시켜 `needs_review`. 둘 다 통과·둘 다 미달이면 요청 밴드에 **더 가까운 쪽**, 동점이면 `attempt_no` 작은 쪽. `state_schema_version`은 **`v1` 유지**. **잔여는 `langgraph_state.md` §2.4 코드블록에 `fallback_ref`·`difficulty_regen_used` 2줄 추가 리뷰뿐**([`09`](09_integration_proposals.md) §1-9 A-7 · §2-4.6) | [`06`](06_quality_gates.md) §2 · [`07`](07_refine_policy.md) §4 · `docs/policies/langgraph_state.md` §2.4 |
...
대조 트랙(**T1 문법**의 조항 ID · **T2 어휘 문항**의 표제어)의 R-1 대조는 **직접 조회**라 벡터 검색이 필수가 아니다. 따라서 **T1 vertical slice는 GraphRAG 검색 없이도 완주 가능**하며, GraphRAG의 실효는 T2 본문·T3(문서 코퍼스 근거 검색)에서 나온다. 다만 **계약(P0-3·P0-4)은 T1 구현 전에 확정**해야 워크플로 재작성을 피한다 — 계약 선행, 검색 백엔드는 후행이다.
...
### 7.3 대조 트랙 자료 — T1 문법 · T2 어휘 문항

> `[2026-08-04]` 종전 "T1 어휘·문법"에서 어휘가 T2로 이관됐다([`05`](05_problem_generation.md) §1). 자료 목록은 그대로이며 **어문 규범은 T1, 사전은 T2 어휘 문항이 소비한다**.

#### 필수

1. **국립국어원 어문 규범 버전 고정본** — T1 문법
   - 한글 맞춤법, 표준어 규정, 표준 발음법
   - 범위 확장 시 외래어 표기법·국어의 로마자 표기법
   - 수집일·원문 URL·파일 hash·적용 버전
   - **`grammar_rule` ID는 공식 조항 번호를 그대로 쓴다** — 별도 체계와 대응표를 만들지 않는다. 공공데이터포털의 어문규범 규정 정보가 **항별연혁정보**를 포함하므로 개정 이력이 원본에서 따라온다(05 §1.1)
2. **표준국어대사전**
