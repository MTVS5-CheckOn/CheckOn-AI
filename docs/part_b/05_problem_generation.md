...
>
> **변경 이력**
> - v2.6 (2026-08-10): **T3 만료 작품 풀 선택 경로를 개방했다.** 종전의 “T3는 BE 라이선스 종속” 판단은 저작권이 살아 있는 작품과 보호기간이 끝난 작품을 구분하지 않아 지나치게 넓었다(㊩). 만료 작품 5편은 정식 개방 경로·고정 리비전·본문 해시를 검증해 패키지에서 선택하고, 살아 있는 작품은 계속 BE 라이선스 확인에 종속된다.
> - v2.5 (2026-08-10): **T2 본문 지문 생성 경로 구현을 §1.2에 반영했다.** `PassageRequest.area_tag`가 `reading` 전용이므로 T4·T5는 다영역 요청 계약 확장 전까지 열리지 않고, T3는 BE 라이선스 원문 풀에 계속 종속된다.
> - v2.4 (2026-08-09): `AreaTag`의 출제 규격 기준 5영역 전환을 반영해 T4를 `speech_writing`으로 맞추고, 자료 조달 축인 T4·T5 트랙 구분은 유지했다.
> - v2.3 (2026-08-05): **라이선스 판정의 전제를 §1.1.4 머리에 못 박았다 — 부트캠프 대회 출제용이며 상업 서비스가 아니다.** 그 전제에서 AI Hub 71857을 **사용 가능**으로 재판정했다(이용정책이 협의를 요구하는 대상은 상업 이용·판매이고 비상업 연구개발은 허용한다). 상용 전환 시 W16·W18이 되살아난다는 조건을 함께 적었다. 한국어기초사전 덤프 실측도 추가 — **LMF 형식(세 사전 스키마가 전부 다름)** · 11파일 중 4개가 아랍어 필드의 백스페이스 7글자로 XML 1.0 위반(재다운로드로 해결 안 되고 필터가 필요) · **어휘 등급 4단계**는 난이도 밴드 외부 기준으로 쓸 값이다.
> - v2.2 (2026-08-05): **§1.1.4 자료 인벤토리 신설 — 외부 조달 일단락.** API 4종·덤프 3종·교육과정·AI Hub 71857 확보. API 판이 3가지로 갈리는 것(우리말샘·기초사전은 `sense_no`가 비고유)과 AI Hub 71857 라벨 실측(문항 1,822 · 성취기준 30종 · 지문 2,630)을 적었다. **AI Hub는 상업 이용에 구축기관 협의가 필요해 v1에서 쓰지 않는다(W18)** — 9.1·9.4·C-15·F17을 한 번에 닫을 수 있는 자료라 아프지만 협의 전에는 보관까지만 한다.
> - v2.1 (2026-08-05): **사전 전체 덤프 3종 확보 · 그러나 R-1은 API만 쓴다(§1.1.3).** 표준국어대사전 색인을 실측하니 99.8 MB(뜻풀이 본문 20.7 MB)라 동봉 선을 넘고, 사전 본문 재배포가 되어 뜻풀이 비복제 방침과 충돌한다. **API 장애 시 어휘 대조가 멈추는 것을 감수한다** — §1.1 fail-closed 그대로다. 남는 재현성 갭(API에 자료 버전이 없어 VersionSet에 적을 값이 없다)은 W17로 등록했다. 덤프의 `word`가 `쓰다01` 형태라 API(`word`+`sup_no` 분리)와 다른 것도 실측해 적었다.
> - v2.0 (2026-08-05): **어휘 자료 확보 — §1.1.3 신설.** 표준국어대사전 오픈 API 실 왕복 확인. `sense_code`가 `dict_entry` 근거 참조의 정본임을 실측으로 확정했다(`쓰다`는 표제어 6개 → 표제어만으로는 의미가 특정되지 않는다). CC BY-SA 동일조건변경허락 때문에 뜻풀이를 산출물에 복제하지 않고 참조만 남기는 방침을 명시했다. `LexiconLookup` 포트 + `StdictClient` 구현, 조회 실패는 `LexiconUnavailable` → 발행 차단. §1.1의 D-03 상태를 자료 확보 완료로 내렸다.
> - v1.9 (2026-08-05): **어문 규범 자료를 패키지에 동봉하도록 §1.1.1을 정정.** 종전 안내는 저장소 밖 절대 경로(`GRAMMAR_NORM_PATH`)를 전제했는데, 그러면 A의 macOS·CI·배포 서버에서 전부 경로가 없어 **R-1 대조가 죽고 T1 전체와 T2 어휘가 발행 차단**된다 — 개발 PC에서만 도는 구조였다. 공공누리 제1유형이 재배포를 막지 않으므로 26MB를 `data/grammar_norm/{버전}/`에 동봉하고, 없으면 즉시 실패하는 `grammar_norm_dir()`과 계약 테스트 7건으로 고정했다.
> - v1.8 (2026-08-05): **§1.1.1 문법 자료 확보 완료 · §1.1.2 자료 권리 표시 신설.** 한국어 어문규범 규정 정보(CSV 8종)를 확보했고 항 1,801개 중 1,560개에 본문이 있다. **`grammar_rule` ID에 시행일이 필요함을 실측으로 확인** — 트리 경로만 쓰면 항 본문 1,560개의 고유 경로가 466개뿐이라 같은 조항의 고시 버전이 서로를 덮어쓴다. 라이선스는 공공누리 제1유형(상업 이용·변형 가능 · 출처 표시 의무)이며 **표시 위치를 서비스 소개·라이선스 페이지로 확정**했다(문항 화면 제외 · `09` §3 W15).
> - v1.7 (2026-08-04): **§1.2 재작성 — 구현 단위는 트랙이 아니라 자료 조달 방식이다.** 종전 §1.2가 `T1 → T2 → T3·T4·T5` 착수 순서를 담아 **트랙을 개발 단위로 오독하게 만들었다.** 실측상 게이트(R-1~R-7)와 프롬프트(`pg.items.v1`·`pg.cross_solve.v1`)는 **전 영역 공용 단일 구현**이고, `area_tag`를 변수로 받는다. 제품 와이어프레임에도 트랙이라는 개발 개념이 없다 — 강사는 약점 지도의 칸을 골라 한 세트에 섞어 담고 워크플로는 문항 단위로 돈다. **구현해야 하는 것은 §1.0 자료 조달 3분류뿐이며, 생성 노드 1개가 T2 본문·T4·T5를 동시에 연다.** `workflow.py`의 가드 문구도 트랙 제한이 아니라 자료 조달 제한으로 정정했다.
> - v1.6 (2026-08-04): §2.1에 **T2 사실성 v1 범위를 확정**했다 — 개념 설명형만 생성하고 사실 서술형은 만들지 않는다. 1층(생성 제약)과 3층(`source_unverified` 차단)이 구현돼 있어 생성 단계에서 사실 서술형을 배제하면 2층 없이 닫히므로, **v1에서 D-04 자료 확보는 선행 조건이 아니다.** 근거는 6월 모평 독서 지문 3종이 전부 개념 설명형이라는 실측이다.
...
| **T3. 문학** | **공통** | `literature` | **저작물** — 만료 작품 풀 선택 · 생존 작품은 BE 라이선스 | 17 (18~34) | ★ 작품 해석 개입 — 교차 풀이 + 검토 배지 강화 |
| **T4. 화법·작문** | 선택 「화법과 작문」 | `speech_writing` | **생성** | 11 (35~45) | ★ 담화 규범·수사 판단 |
...
| **저작물** | T3 | 보호기간이 끝난 작품은 정식 개방 경로·고정 리비전·본문 해시를 검증한 동봉 풀에서 선택한다. **만료 작품에는 BE 라이선스 종속이 없다.** 저작권이 살아 있는 작품만 BE 라이선스 확인에 계속 종속된다. **작품 텍스트 변형 절대 금지**(§3) — LLM이 원문을 생성하면 수능 문학이 아니다 |
...
### 1.1 대조 트랙 기준 자료 요건 `[자료 확보 완료 — 2026-08-05]`

> D-03의 두 축이 모두 닫혔다. **문법**은 어문 규범 CSV 동봉(§1.1.1), **어휘**는 표준국어대사전 API(§1.1.3). 남은 OPEN은 자료가 아니라 대조 스냅숏 영속화(A-1)와 라이선스 전파 확인(`09` §3 W16)이다.
...
- **`grammar_rule` ID는 별도 체계를 만들지 않는다** — 어문 규범의 조항 번호를 그대로 쓴다. 규정 정보 데이터가 **항별연혁정보**를 포함하므로 개정 이력이 원본에서 따라온다.
...
#### 1.1.1 문법 자료 확보 완료 `[2026-08-05]`

**한국어 어문규범 규정 정보**(공공데이터포털 · 문화체육관광부 국립국어원)를 확보했다. 파일데이터 8종 CSV이며 신청 없이 내려받는다.

| 파일 | 행 | 역할 |
| --- | ---: | --- |
| **최신고지규정정보** | 2,198 | **조항 원문 + 해설 — R-1 대조 정본** |
| 발행고시규정정보 | 2,198 | 고시 시점 스냅숏(개정 전 버전) |
| 용례정보 | 80,404 | 올바른 표기 / 오표기 쌍 |
| 관련규정정보 | 83,992 | 조항 간 참조 |
| 항별연혁정보 · 결정근거 · 북한규정정보 · 고시정보 | 1,081 | 개정 이력·근거·대조·메타 |

**항 1,801개 중 1,560개에 본문이 있다.** 규정코드 분포는 외래어 표기법 1,348 · 한글 맞춤법 634 · 표준어 규정 157 · 국어의 로마자 표기법 59다.

**🔴 `grammar_rule` ID에 시행일이 필요하다 — 실측으로 확인했다.** 트리 경로만 쓰면 유일하지 않다. 항 본문 1,560개의 경로를 뽑으면 **고유 경로가 466개뿐이고 1,094개가 중복**이다. 같은 조항이 고시 버전마다 행으로 들어 있기 때문이다.

```
형식   어문규범:{시행일}:{규정코드}:{트리경로}
예시   어문규범:2017-03-28:한글 맞춤법:본문>제3장>제1절>제5항
```

트리 경로는 `최신고지규정정보`의 `상위규정번호`를 거슬러 `제목`을 이어 붙여 만들고, 시행일은 `고시정보.csv`의 `고시번호`로 조인해 얻는다. **시행일을 빼면 같은 조항의 여러 고시 버전이 서로를 덮어쓴다.**

⚠ 원본 `관련규정정보_UTF8.csv`는 파일명과 달리 실제 **cp949**다. 나머지 7종은 UTF-8이다. 저장소 동봉본은 전부 UTF-8(BOM 없음)로 통일했다.

**자료는 패키지에 동봉한다 — 외부 경로를 쓰지 않는다.**

```
src/ai/problem_generation/data/grammar_norm/nikl-kornorms-20250902/   26 MB · CSV 8종 + SOURCE.txt
```

`verify_config.yaml`과 같은 방식이다(`infrastructure/config.py`의 `_DATA_ROOT` 상대 경로). **환경별 절대 경로나 볼륨 마운트를 쓰면 개발 PC에서만 돌고 서버에서 죽는다** — 자료가 없으면 R-1 대조가 불가능해 T1 전체와 T2 어휘 문항이 발행 차단되므로(§1.1), 배포에서 빠지는 순간 서비스가 서지 않는다. 공공누리 제1유형은 재배포를 막지 않으므로 저장소 동봉에 제약이 없다.

`grammar_norm_dir(version)`은 디렉터리가 없으면 **즉시 실패한다**. 빈 경로를 돌려주면 R-1이 대조 없이 통과하는 길이 생긴다(불변식 2). `tests/ai/contract/test_grammar_norm_bundled.py` 7건이 동봉·판독·본문 실존·라이선스 기록을 고정한다.

**버전 폴더명이 곧 자료 버전이다.** 자료가 개정되면 새 폴더를 추가하고 `DEFAULT_GRAMMAR_NORM_VERSION`을 올린다 — 옛 버전이 남아야 과거 문항의 재검증이 재현된다(불변식 8). `GRAMMAR_NORM_PATH` env는 쓰지 않는다.

#### 1.1.2 자료 권리 표시 — 의무이며 위치를 정한다

**모든 기준 자료는 라이선스와 출처 표시 의무를 `SOURCE.txt`에 함께 보관한다.** 자료 폴더에 원문 표기·공식 명칭·근거 URL·허용 범위·표시 문구를 남긴다 — 나중에 "왜 이 자료를 상업 이용해도 되는가"에 답할 수 있어야 한다.

**어문 규범 자료의 라이선스**

```
원문 표기   "공공저작물 : 출처표시 (제 1유형)"   (data.go.kr 이용허락범위)
공식 명칭   공공누리 제1유형 · https://www.kogl.or.kr/info/licenseType1.do
허용       상업적·비상업적 이용 · 변형 등 2차적 저작물 작성
의무       출처 표시 (발행연도 · 기관명 · 홈페이지 URL 포함)
```

**노출 위치 `[2026-08-05 확정]` — 서비스 소개·라이선스 페이지에 모아 표시한다. 문항 화면에는 띄우지 않는다.** 공공누리는 표시 의무만 규정하고 위치를 지정하지 않으며, 문항마다 띄우면 강사 화면이 법적 고지로 오염된다. 자료가 늘어날수록 한곳에 모으는 편이 누락도 적다.

**FE가 그 페이지를 만들어야 한다** — [`09`](09_integration_proposals.md) §3 W15.

`LICENSE_REF` env 값(`KOGL-1` 등)은 **저장소 내부 식별자**이며 공식 명칭이 아니다. 공식 표기는 `SOURCE.txt`가 정본이다.

#### 1.1.3 어휘 자료 확보 완료 `[2026-08-05]`

**국립국어원 표준국어대사전 오픈 API**(`https://stdict.korean.go.kr/api`)를 확보했다. 인증 키를 발급받아 쓰며 실 왕복을 확인했다.

**어문 규범과 달리 파일 동봉이 아니라 API 조회다.** R-1 어휘 대조는 *생성된 문항에 나온 단어*만 확인하는 점 조회라 전수(50만 표제어)를 가질 이유가 없다. 문법은 조항 트리 전체를 훑어야 해서 파일이 맞았고, 어휘는 조회가 맞다.

**🔴 `sense_code`가 대조 단위다 — 표제어로는 특정되지 않는다.** 실측으로 확인했다. `쓰다`는 표제어가 6개고 표제어마다 다시 여러 의미를 가진다. 어휘 문항은 "문맥상 ⓐ와 가장 가까운 의미"처럼 **어느 의미인지**를 묻기 때문에(2027 6월 모평 9·17번), 근거를 의미 단위로 걸지 않으면 대조가 성립하지 않는다.

```
형식   표준국어대사전:{sense_code}
예시   표준국어대사전:484613     (쓰다1 — 머릿속의 생각을 글로 나타내다)
```

`grammar_rule` ref가 `어문규범:{시행일}:{규정코드}:{경로}`인 것과 같은 자리다(§1.1.1). 사전은 의미코드 자체가 고유해 판이 더 짧다.

**API 실측 특성** — 문서와 다른 부분이 있다.

| 확인 | 내용 |
| --- | --- |
| `view.do`는 **XML만** 응답 | `req_type=json`을 줘도 본문이 빈 문자열(HTTP 200). 문서엔 JSON 예시가 있으나 나오지 않는다 |
| `sense_code`는 `view.do`에만 | `search.do` 응답엔 `sense_no`뿐이라 의미를 전역 식별할 수 없다 → **2단 조회**(`search.do`로 `target_code` → `view.do`로 의미) |
| 결과 없음은 오류가 아님 | `total=0`으로 온다. R-1의 **"표제어 없음"** 판정이 여기서 성립한다 |
| 텍스트 노드에 개행·들여쓰기 혼입 | `word_unit` 등. 전부 공백 접어서 쓴다 |

**🔴 라이선스는 어문 규범보다 세다 — CC BY-SA.**

```
상업적 이용   가능 ("상업적 용도까지 포함하여 자유롭게")
라이선스     CC BY-SA 2.0 KR
동일조건     "변경하여 새로운 저작물을 만들 때, 그 저작물도 동일한 라이선스로 배포"
출전 용례    원저작자 별도 허락 필요
```

**그래서 뜻풀이를 산출물에 복제하지 않는다.** `dict_entry` 근거 앵커는 `ref`만 남기고 `quote`는 `null`이다 — 계약상 `quote`는 `passage_span`·`work_span`에만 필수라 그대로 성립하고, 프롬프트 규칙(`items.txt`)과도 일치한다. 참조는 대조 근거로 충분하며 2차 저작물 작성이 아니다. **출전이 붙은 용례는 쓰지 않는다.**

⚠ 동일조건변경허락이 상용 SaaS 산출물에 전파되는지는 **법적 확인이 남아 있다**(`09` §3 W16). 위 비복제 방침은 회피책이지 판정이 아니다.

**구현**

```
domain/lexicon.py            LexiconEntry · LexiconSense · dict_entry_ref · dict_entry_anchor   (순수)
application/ports.py         LexiconLookup 포트 · LexiconUnavailable
infrastructure/stdict.py     StdictClient · StdictSettings                                      (httpx + XML)
```

사전이 바뀌어도(→ 우리말샘) 규칙 코드가 그대로여야 하므로 포트로 끊었다. **조회 실패를 빈 결과로 삼키지 않는다** — `LexiconUnavailable`은 `verification_unavailable`로 올라가 발행이 차단된다(§1.1). "없는 말"과 "못 물어봄"을 같게 취급하면 R-1이 대조 없이 통과한다(불변식 2). 조회당 상세 호출은 `stdict_max_entries`로 상한을 둔다(불변식 6).

접속값은 env 주입 — `STDICT_API_KEY` · `STDICT_BASE_URL`. 테스트는 단위 12건(`test_lexicon.py`) + 실 API 4건(`test_stdict_smoke.py`, `-m integration`).

**전체 내려받기는 확보했으나 쓰지 않는다 `[2026-08-05 결정]`**

사전 3종의 전체 덤프를 받아 `local_data/`에 뒀다(저장소 반입 금지 — `.gitignore`). **R-1은 API만 쓴다.**

| 덤프 | 크기 | 파일 |
| --- | ---: | ---: |
| 표준국어대사전 `20260805` | 657 MB | 88 |
| 우리말샘 `20260802` | 1.74 GB | 25 |
| 한국어기초사전 `20260719` | 369 MB | 11 |

표준국어대사전 덤프로 대조용 SQLite 색인을 만들어 실측했다 — **의미 509,138건 · 표제어 436,582개 · 99.8 MB**(gzip 38.7 MB, 뜻풀이 본문만 20.7 MB). 동봉하지 않기로 한 이유는 둘이다. ① 어문 규범 26 MB는 동봉이 맞았지만 **100 MB는 선이 다르다** — clone마다 따라다닌다. ② 뜻풀이 20.7 MB를 저장소에 넣는 것은 **사전 본문 재배포**라, 산출물에 뜻풀이를 복제하지 않기로 한 방침(위)과 앞뒤가 맞지 않고 W16을 날카롭게 만든다.

**대가는 명시한다 — API가 죽으면 R-1 어휘 대조가 멈추고 T2 어휘 문항이 발행되지 않는다.** 이건 결함이 아니라 §1.1의 fail-closed 그대로다. 자료 없이 통과시키는 것보다 멈추는 편이 낫다.

⚠ **재현성 갭이 남는다(불변식 8).** API에는 자료 버전이 없어 `VersionSet`에 기록할 값이 없다 — 사전이 개정되면 같은 문항이 어제는 통과하고 오늘은 실패할 수 있다. `09` §3 W17로 등록했다.

**덤프는 API와 형식이 다르다 — 실측.** 나중에 적재할 때 정규화가 필요하다.

```
API    word="쓰다"     sup_no="1"
덤프    word="쓰다01"   sup_no=(509,138행 전부 빔)
```

`sense_code`는 양쪽이 같다 — API가 준 `484613`·`481118`이 덤프 색인에서도 같은 뜻풀이로 조회된다. **근거 참조 체계는 덤프로 옮겨도 그대로 성립한다.**

**남은 것**

- **대조 스냅숏 영속화** — 재검증 때 재조회하지 않으려면 조회 결과를 캐시해야 한다(불변식 8). 저장 계층이라 A-1과 함께 정한다.
- **어휘 후보 풀** — "어떤 단어로 낼까"를 고르려면 전수가 필요하다. 사이트의 전체 내려받기를 쓴다. A3(자료 생성 노드) 이후.
- **우리말샘·한국어기초사전** — 표준국어대사전으로 먼저 돌려 보고 필요분만 추가한다.

#### 1.1.4 자료 인벤토리 `[2026-08-05 조달 완료]`

외부 자원 확보를 한 차례에 끝냈다. **저장소에는 어문 규범과 저작권 만료 문학 풀만 들어가며, 나머지는 `local_data/`(반입 금지) 또는 env 접속값이다.**

> **🔴 라이선스 판정의 전제 — 이 절의 모든 판단이 여기에 걸려 있다 `[2026-08-05]`**
>
> 이 프로젝트는 **부트캠프 대회 출제용이며 상업적 서비스·판매가 아니다.** 자료 라이선스 판정은 전부 이 전제 위에서 내렸다.
>
> **상용으로 전환하면 결론이 바뀐다.** 최소한 다음이 되살아난다 — `W16`(CC BY-SA 전파 여부) · `W18`(AI Hub 구축기관 협의) · 사전 뜻풀이를 산출물에 복제하는 범위. **전환 시점에 이 절과 `09` §3을 다시 읽는 것이 조건이다.**
>
> 전제가 바뀌면 결론도 바뀐다는 사실을 남기는 것이 목적이다. 나중에 "왜 괜찮다고 했지"가 나오지 않게 한다.

| 자원 | 어디에 | 크기 | 라이선스 | v1 사용 |
| --- | --- | ---: | --- | --- |
| 어문 규범 CSV 8종 | **패키지 동봉** | 26 MB | 공공누리 1유형 | ✅ R-1 문법 |
| 저작권 만료 문학 풀 5편 | `src/ai/problem_generation/data/literature_pool/` | 64,503 B | 보호기간 만료·저작권 성립 이전 원문 | ✅ T3 — `source_ref` oldid·`revision_id`·본문 sha256 고정 |
| 표준국어대사전 API | `STDICT_*` | — | CC BY-SA | ✅ R-1 어휘 |
| 우리말샘 API | `OPENDICT_*` | — | CC BY-SA | 대기 — `sense_no` 비고유 |
| 한국어기초사전 API | `KRDICT_*` | — | 미확인 | 대기 — **5만 회/일** |
| 어문 규범 용례 API | `KORNORM_EXAMPLE_*` | — | 공공누리 1유형 | 대기 — 동봉 CSV와 동일 내용 |
| 사전 덤프 3종 | `local_data/` | 2.8 GB | 위와 같음 | ❌ §1.1.3 — A-1 대기 |
| 2022 개정 교육과정 국어과 | `local_data/교육과정/` | 3.9 MB | 공공누리 추정(확인 필요) | 🔴 `9.1` 근거 |
| AI Hub `71857` 국어 교과 지문형 문제 | `local_data/` | 281 MB | 상업 이용만 협의 필요 | 🟢 사용 가능(비상업 전제) |

**API 4종의 판이 3가지로 갈린다** — 어댑터를 공유할 수 없다.

| | 엔드포인트 | 인증 변수 | 의미 ID |
| --- | --- | --- | --- |
| 표준국어대사전 | `search.do` `view.do` | `key` | `sense_code` **전역 고유** |
| 우리말샘 · 기초사전 | `search` `view` (`.do` 없음) | `key` | `sense_no` — 표제어 내 순번, **비고유** |
| 어문 규범 용례 | `exampleReqList.do` | `serviceKey` + 페이지네이션 필수 | 해당 없음 |

우리말샘·기초사전을 붙일 때 근거 참조는 `{사전}:{target_code}:{sense_no}` 조합으로 가야 한다 — 표준국어대사전처럼 의미코드 단독으로는 특정되지 않는다.

**AI Hub `71857`은 우리가 막혀 있던 구멍을 정확히 겨냥한다 — 그래서 라이선스가 아프다.** 라벨 전량을 열어 실측했다.

```
문항 1,822건          고1 1,350 · 고2 472
구성요소              문항 1,820 · 지문 1,813 · 정답 1,817 · 오답 1,804 · 해설 1,821
                     지문(이미지) 140 · 원천은 PNG 1,822장 + Bounding_Box 좌표
난이도                하 1,287 (71%) · 중 441 · 상 94
2022 성취기준 30종     고등 19종 1,489건 · 중등 11종 655건
지문 2,630개          중위 705자 · 최대 21,624자
```

닫을 수 있는 것 — **전부 "경로가 없다"고 적어둔 항목들이다.**

| 구멍 | 이 자료가 주는 것 |
| --- | --- |
| `9.1` `curriculum_graph.yaml` 근거 | 성취기준 코드 ↔ 실제 문항 매핑 1,489건. 맨손으로 짜면 근거가 우리 감이었다 |
| `9.4` 교사 검수 골든셋 | 정답·오답·해설이 라벨로 붙어 있다 |
| `C-15` R-8 외부 대조 코퍼스 | 지문 2,630개 — §8.3이 "대조 수단 미확정"이라 적은 그 자리 |
| 난이도 밴드 외부 기준 | 하/중/상 라벨. ⚠ **하가 71%로 심하게 치우쳐** 그대로는 못 쓴다 |
| `F17` 지면 시험 OCR | PNG + Bounding_Box. Phase 2 학습·검증 데이터 |

**비상업 전제에서는 쓸 수 있다 `[2026-08-05 재판정]`.** AI Hub 이용정책이 협의를 요구하는 대상은 **상업적 이용·판매**이고 **비상업적 연구개발은 허용**한다. 대회 출제용은 후자다 — 그래서 위 다섯 구멍을 이 자료로 닫는다.

⚠ **상용 전환 시 `W18`이 되살아난다.** 그때는 구축기관 협의가 선행 조건이 되고, 협의가 안 되면 `9.1`·`9.4`는 사람 작업으로 돌아가고 `C-15`는 다시 빈다.

⚠ 중등 성취기준 655건이 섞여 있다. `CLAUDE.md`가 중등 분기를 금지하므로 **쓰게 되더라도 고등 19종 1,489건만** 쓴다.

**한국어기초사전 덤프는 형식이 또 다르다 — LMF 표준이다.** 세 사전 덤프의 스키마가 전부 달라서 파서를 공유할 수 없다.

| 덤프 | 구조 | 의미 ID |
| --- | --- | --- |
| 표준국어대사전 | `channel > item > word_info` | `sense_code` 전역 고유 |
| 우리말샘 | `channel > item > wordInfo` (camelCase) | `sense_no` 비고유 |
| 한국어기초사전 | `LexicalResource > Lexicon > LexicalEntry > Sense` · 값은 전부 `<feat att= val=>` | `Sense@id` 표제어 내 순번 |

🔴 **원본 11파일 중 4개가 XML 1.0 위반이다 — 실측.** `\x08`(백스페이스, U+0008) 7글자가 **아랍어 대역어 필드**에 박혀 있다. XML은 이 문자를 이스케이프해도 허용하지 않으므로 표준 파서가 파일 전체를 거부한다.

```
1_5000 2개 · 2_5000 1개 · 3_5000 1개 · 5_5000 3개   (총 7개)
```

**재다운로드로 해결되지 않는다 — 배포 원본 자체가 그렇다.** 제어문자를 걸러내면 11/11 판독되고 표제어 56,555개를 얻는다. 우리는 한국어만 쓰므로 아랍어 필드 손상은 실질 영향이 없다. **파서에 XML 1.0 금지 문자 필터를 넣는 것이 조건이다** — 없으면 자료의 36%가 조용히 사라진다.

🟢 **어휘 등급이 붙어 있다 — 표준국어대사전에 없는 값이다.**

```
고급 37,024 · 중급 9,068 · 초급 2,545 · 없음 5,034
품사  명사 30,227 · 동사 11,180 · 형용사 2,961 · 부사 2,859
```

**난이도 밴드의 외부 기준으로 쓸 값이다.** AI Hub 난이도 라벨은 하가 71%로 치우쳐 그대로 못 쓰는데(위) 이쪽 분포가 훨씬 낫다. 연결 방식은 `06` §3 난이도 추정과 함께 정한다.

### 1.2 구현 단위는 트랙이 아니라 자료 조달 방식이다

> **`[2026-08-04 정정]`** 이 절은 종전에 `T1 → T2 → T3·T4·T5` 착수 순서를 담고 있었다. **그 표기가 트랙을 개발 단위로 오독하게 만들었다.** 트랙은 **문항 유형 분류**이고, 구현에서 갈리는 것은 §1.0의 **자료 조달 3분류** 하나다. 제품 와이어프레임에도 트랙이라는 개발 개념이 없다 — 강사는 약점 지도의 칸(영역×유형)을 골라 **한 세트에 섞어 담고**(`선택과목이 아닌 칸도 담을 수 있습니다 — 경고만 표시`), 워크플로는 **문항 단위**로 돈다.

**게이트와 프롬프트는 전 영역 공용이다 — 트랙별로 만들지 않는다.**

```
items.txt        pg.items.v1        area_tag를 변수로 받아 전 영역 처리
cross_solve.txt  pg.cross_solve.v1  〃
R-1 ~ R-7        domain/rules.py    트랙 무관 단일 구현
```

**구현해야 하는 것은 자료 조달 3분류뿐이다.**

| 자료 조달 | 필요한 것 | 여는 트랙 | 상태 |
| --- | --- | --- | --- |
| **자료 없음** | — | T1 | ✅ 구현 |
| **생성** | 자료 생성 노드(지문·담화·매체 자료를 LLM이 만든다 — `passage.txt` 계열) | **T2 본문** | ✅ T2 본문 경로 구현 · 승인 앵커 제공 필요 |
| **생성 계약 확장** | `PassageRequest`를 화법과작문·매체 입력까지 다영역화 | **T4 · T5** | ❌ 요청 계약 미구현 |
| **저작물** | 만료 작품 풀 선택 노드 1개 | T3 | ✅ 구현 · 생존 작품만 BE 라이선스 종속 |

생성 프롬프트·게이트는 세 트랙에서 재사용할 수 있지만, 현재
`PassageRequest.area_tag`는 `reading`만 허용하므로 **T2 본문만 열려 있다.** T4·T5는
`SUPPORTED_AREAS`만 늘려 우회하지 않고, 영역별 생성 입력을 포함한 요청 계약을 먼저
확장해야 한다.

**자료 확보가 필요한 곳(§1.0 대조 트랙)**

| 대상 | 필요 자료 | 없으면 |
| --- | --- | --- |
| T1 문법 | 어문 규범 | R-1 대조 불가 → 발행 차단 |
| T2 어휘 문항 | 사전 | 〃 |
| T3 문학 | 만료 작품 풀은 패키지 동봉 · 생존 작품은 BE 라이선스 | 만료 작품은 개방 · 생존 작품은 라이선스 없으면 미지원 |

**T2 본문·T4·T5는 자료 확보가 필요 없다** — 생성 트랙이라 자료를 만들기 때문이다(§1.0).

**게이트를 검증 없이 열지 않는다.** 이건 트랙 순서 규율이 아니라 **자료 조달 방식별 규율**이다. 새 조달 방식을 열 때 그 방식의 R-1 검증 경로(생성=자료 내부 근거 · 대조=외부 정본 · 저작물=발췌 범위)와 골든을 함께 연다.
...
> **v1 범위 확정 `[2026-08-04]` — 개념 설명형만 생성하고 사실 서술형은 만들지 않는다.**
>
> 1층(생성 제약)과 3층(`source_unverified` 차단)이 구현돼 있으므로, **생성 단계에서 사실 서술형을 아예 만들지 않으면 2층(D-04 승인 자료)이 없어도 닫힌다.** 따라서 **v1에서 D-04 자료 확보는 선행 조건이 아니다.**
>
> 근거 — 2027학년도 6월 모의평가 독서 지문 3종(조선 노비의 재산권·완전경쟁시장의 정보 획득·구형과 표면장력)이 전부 **특정 연구를 인용하지 않는 개념 설명형**이다. 수능 독서 지문 형식과 1층 제약이 크게 충돌하지 않는다.
>
> **v2로 넘기는 것** — 사실 서술형 지문과 그 근거 자료 체계(D-04). 열 때 2층을 함께 연다.

## 3. 문학 풀 — T3 선택 규칙

- **`[2026-08-10 ㊩ 정정]`** 종전에는 모든 T3 작품을 BE 라이선스 풀 종속으로 적었지만, 이는 보호기간이 끝난 작품과 저작권이 살아 있는 작품을 갈라 보지 않은 좁은 판단이었다. 동봉한 만료 작품에는 BE 라이선스 종속이 없고, 생존 작품을 추가하는 경우에만 BE의 `license_ref` 확인 계약이 다시 선행한다.
- 만료 작품 풀은 `data/literature_pool/index.json`의 정식 개방 경로·고정 리비전·본문 sha256와 `.txt` 파일의 정확한 대응을 로딩에서 검증한다. 사망연도가 있으면 수집연도 기준 사후 70년 경과를, 미상이면 비어 있지 않은 사유를 요구한다.
- 선택 로직은 결정론: `WorkSelection`의 갈래·시대·개념어로 후보를 좁히고 **slug 정렬** 뒤 요청의 canonical JSON 해시로 작품과 구간을 고른다. `index.json` 배열 순서는 판정 입력이 아니다. LLM은 발췌 범위 안에서 문항만 생성한다.
- **작품 텍스트 변형 절대 금지** — 발췌는 오프셋 범위 지정으로만, 고전 현대어 풀이는 풀 등재 풀이본만(버전·license_ref 저장).

## 4. 계약 스키마 — contracts/problem_generation.py (B 단독)

**enum 규율:** `area_tag`·`type_tag`·`item_format`은 `contracts/taxonomy.py` 공용 enum을 **직접 사용**한다(5영역·B-3 경계 사례 확정). 문자열 자유 입력 금지. `item_format`은 v1에서 `mcq` 고정 — short·essay 분기 코드 작성 금지(CLAUDE.md §3).
