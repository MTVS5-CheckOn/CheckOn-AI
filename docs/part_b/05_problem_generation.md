...
| --- | --- | --- | --- |
| **자료 없음** | — | T1 | ✅ 구현 |
| **생성** | 독서 `PassageRequest` + 화법과작문·매체 형제 요청 모델. `area_specs.yaml` 규격과 승인 앵커로 자료를 생성한다 | **T2 · T4 · T5** | ✅ 구현 · evidence 없는 자료는 생성 단계에서 차단 |
| **저작물** | 만료 작품 풀 선택 노드 1개 | T3 | ✅ 구현 · 생존 작품만 BE 라이선스 종속 |

`PassageRequest.area_tag=reading`은 그대로 유지한다. T4는 발표·초고·수집 자료 형태를,
T5는 단일·쌍 매체 자료 형태를 갖는 형제 요청 모델로 분리했으며, 세 생성 트랙 모두 같은
승인 근거·금칙·R-1 실패 닫힘 규율을 적용한다.
