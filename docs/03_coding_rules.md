...
- **모든 LLM 호출은 `llm/gateway` 경유 — 예외 없음.** provider(벤더 어댑터) 직접 호출 금지(`llm/` 내부 제외). 직결하면 role 라우팅·원가 기록·redaction 훅이 소비자마다 갈라진다. 근거: `part_b/01_pipeline.md §5`(원 규칙 — B 소유, 참조만) · `part_b/09_integration_proposals.md §1-10`(A·B 공용 승격 합의).
