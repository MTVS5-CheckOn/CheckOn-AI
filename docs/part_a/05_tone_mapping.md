...
- `buffer_lexicon.yaml`: §4 — A군 검출은 형태소 접두 매칭(활용형 대응).
  > 🔴 **현행 구현은 이 규정에 미달한다(8/4 실측).** 실제 판정은 **부분 문자열 포함**이라
  > 어간 끝 음절에 종성이 없는 **A군 8항**은 종성이 결합한 활용형(`게으른`·`게으름`·
  > `산만한`·`머리가 나쁜`)을 전부 놓치고, 8/4 실 LLM 스모크에서 refine이 `게으른`으로
  > 실제로 뚫렸다. **규정은 그대로 둔다 — 고쳐야 할 것은 구현이지 규정이 아니다.**
  > 범위·해소안 판단과 수용 기준은 **`99_open_items.md` D ⑰이 정본**이다.
