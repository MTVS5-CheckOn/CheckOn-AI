    ...
    "signals": [
      ...
      // 백엔드는 순서 그대로 저장하고, 프론트는 순서 그대로 그리면 됩니다
      {
        ...
        "evidence": [
          ...
          // 🔴 (8/14) 상한이 3 → 최대 6으로 늘었다: trigger 최대 3 + baseline 최대 3.
          //    **최신 것부터 남긴다** — 종전에는 오래된 주가 남아 「이번 주」 근거가 잘렸다
          {
            "source_table": "learning_event",
                // [로그] 원본이 어느 테이블인지 힌트
            "record_id": "le_1029",
                // [저장] 백엔드 DB 원본 PK — 강사가 [근거 보기] 클릭하면 이 ID로 원본 조회
            "summary": "acc_drop 근거 기록",
                // [표시] ⚠ (8/14) **deprecated** — 그 안의 숫자를 아래 구조화 필드로 옮겼다.
                //       필드는 지우지 않으니 전환은 편할 때 하면 된다
            "role": "trigger",
                // 🔴 [저장][표시] (8/14 신설) **필수** — trigger(판정을 성립시킨 기록)
                //    | baseline(비교 대상이 된 기록). 표시 층이 「근거」와 「비교 기준」을 가른다
            "observed": null,
                // [표시] (8/14 신설) **그 기록 자신의 값**. learning_event는 문항 단위라
                //    주 단위 지표(정답률)에 해당하는 값이 그 레코드에 없다 → null
            "sample_size": null,
                // [표시] (8/14 신설) 그 기록의 분모
            "occurred_on": "2026-07-03"
                // [표시] (8/14 신설) **그 기록의 날짜**. 주 단위 집계 레코드만 주 시작일이다
          },
          {
            "source_table": "learning_event", "record_id": "le_1077",
            "summary": "acc_drop 근거 기록",
            "role": "trigger", "observed": null, "sample_size": null,
            "occurred_on": "2026-07-08"
          }
        ],

        "metric": "accuracy",
            // [표시] 🔴 (8/14 신설) 이 신호가 무엇을 쟀나.
            //    **단위가 규칙마다 다르다** — submit_drop은 「주 수」라 %로 찍으면 300%가 나간다.
            //    규칙별 단위 정본: `part_a/14_evidence_fields.md` §3-2′ (여기 복제하지 않는다)
        "observed": 0.51,
            // [표시] (8/14 신설) 판정 창 **마지막 주**의 관측값 — 강사가 보는 「지금」이 그 주다.
            //    🔴 evidence[].observed와 **다른 값**이다: 이쪽은 신호 전체, 그쪽은 개별 레코드.
            //    submit_drop에서 이쪽은 「연속 3주」, 그쪽은 「그 주 제출 0건」이다
        "baseline": 0.72,
            // [표시] (8/14 신설) 비교 기준. 🔴 **비교하지 않는 규칙은 null**이다 —
            //    submit_drop·type_bias·return_care는 임계값과 비교하지 「평소」와 비교하지 않는다.
            //    임계를 여기 적으면 「평소 대비」로 읽혀 거짓이 된다
        "sample_size": 20
            // [표시] (8/14 신설) 분모. ⚠ 규칙마다 뜻이 다르다 —
            //    acc_drop은 채점 문항 수, volume_gap은 **기준선 주 수**다
      }
    ],
...
> **화면 표기 각주(7/21 저니맵 리뷰 확정):** `follow_up`의 화면 문구는 **'리마인드'로 통일**한다(옛 표기 '팔로업'보다 사용자에게 직관적). 내부 값 `lifecycle=follow_up`과 쿨다운 2주 동작은 그대로 유지 — 표기만 리마인드, 계약 값·판정 동작 무변경.
