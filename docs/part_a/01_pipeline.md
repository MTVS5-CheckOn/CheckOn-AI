...
> | 보조 AI ⓑ | 문의 분류 (주제·성향·긴급도 3축) | 선형 LLM 분류 |
...
flowchart TB
  ...
  BE["백엔드 (Java·PostgreSQL) — 승인·발송·확정 전부 백엔드 (HITL)"]
  E <--> BE
...
```
문의 원문 → LLM 분류: **3축 독립** — topic(성적|일정|상담요청|기타) + sentiment(normal|complaint) + urgency(즉시|일반) + 축별 confidence
→ 구조화 출력(enum 강제 — 자유 텍스트 분류 불가) → 인박스 정렬·완충 강도·할 일 생성 규칙의 입력
→ 오분류 시 강사가 원탭 수정 → 수정 이력이 분류 품질 평가셋으로 축적
```
**`complaint`는 `topic`이 아니라 `sentiment` 축이다** — "무엇에 대한 문의인가"와 "어떤 감정으로 쓴 문의인가"는 다른 축이고, 축 분리가 업계 표준이다(Zendesk는 Intent·Sentiment·Language·Entity를 독립 필드로 두고 각각 confidence를 단다 · ITIL도 Category ⊥ Impact×Urgency). 종전 표기는 같은 값이 두 축에 중복돼 있었다.

**분류 결과는 되돌릴 수 없는 것을 가르지 않는다.** 표시·정렬·완충 강도·**초안 종류**까지가 사용 범위이고, 학부모에게 나가는 것(자동 응답)에는 쓰지 않는다 — 승인·발송은 전부 HITL이다. 판정 기준은 "피해가 정렬 순서에 그치는가"가 아니라 **"강사가 되돌릴 수 있는가"**다.

> 🔴 **(8/5) 원칙 재작성 — 종전 문구는 문서 자기모순이었다.** 종전은 "피해가 '정렬 순서'에 그치도록 표시·정렬에만 쓴다"였는데, 같은 문서 계열이 `topic=schedule → template_only 경로`를 확정하고 있었다(`part_a/03` §C 상황 2 · `part_a/02` §1-B 도식 · `policies/error_codes` §2.1 · `08_kafka_events` §46). `template_only`는 초안 종류를 가르므로 "정렬 순서"를 넘는다.
>
> **업계 표준은 분류가 생성 경로를 가르는 쪽이다** — Gorgias(intent off → 전건 사람에게), Front Autopilot(선택 Topic만, 다중 Topic이면 미발송), Zendesk auto assist(`Ticket > Intent` 트리거 조건), Intercom Fin Procedures, Salesforce Agentforce Topics가 모두 같은 구조다. 종전 규칙은 "생성 경로 분기"와 "고객 대면 자동응답"을 한 문장에 묶은 과잉 규칙이었다.
>
> 실제 기준은 **되돌릴 수 있는가**이며, 이는 Salesforce Einstein Case Classification의 운용 권고("되돌릴 수 있는 액션은 낮은 임계값, 되돌리기 힘든 액션은 높은 임계값")와 같다. **예외 조항을 파지 않은 이유**는 예외 목록이 늘어나면(urgency는? sentiment는?) 원칙이 무의미해지기 때문이다. 되돌리기는 선언이 아니라 계약 의무다(`04` §3.9 재요청 규약).
...
기존 원칙 유지: 전 테이블 tenant_id + 애플리케이션 계층 격리(RLS 미도입 — 99 BE-11), 도메인 원본은 백엔드 DB, ai_run/llm_call 연결로 재현·비용 추적.
...
| ⓑ | enum 강제 파싱 · **3축 독립성**(한 축 오분류가 다른 축을 오염시키지 않음) · **되돌리기 보장** 계약 테스트(오분류 → 강사 정정 → 새 키 재요청 경로가 존재하는가) |
