...
| ⓕ | 🔴 **완료 통지가 안 오는 경로가 둘** · **202 바디는 언제나 2키** | **ⓐ 202는 `{job_id, status}` 둘뿐입니다** — 초안 실물은 **언제나 GET**입니다(`success_envelope(data={"job_id": …, "status": …})` · GET만 `view.model_dump()`로 `result`를 싣습니다). 04 §3.9도 `202 { "job_id": "cj_1029", "status": "queued" }`로 **2키가 맞습니다.** **ⓑ 🔴 `topic=schedule`(`template_only`)과 인용 가능 근거 0건(`rejected_insufficient`)은 완료 통지가 없습니다** — 그 둘은 `enqueue` **앞에서 반환**해 **`WorkerJob` 행 자체가 안 생기고**, 잡이 없으면 종단 phase 전이가 없어 **통지가 발생할 자리가 없습니다.** ⚠ **Kafka를 배선한 뒤에도 참입니다** — 「아직 미구현이라 어차피 다 안 온다」와 **다른 사실**입니다. ⚠ **계약 정본은 04 §2.2 「202를 받은 BE가 무엇을 하나」**입니다 — 여기는 운영 관점의 재진술입니다. **ⓒ 일반 경로는 동기입니다** — v1은 같은 요청에서 러너를 1회 돌리므로 **202가 이미 `succeeded`인 경우가 많습니다.** `queued`로 오는 것은 그 러너가 큐의 **다른 잡**을 집었을 때입니다(둘 다 정상). <br><br>🔴 **⇒ BE 분기 규칙 한 줄 — 이 하나로 세 경로가 다 덮입니다:**<br>**「202의 `status`가 종단(`succeeded`·`failed`·`cancelled`)이면 통지를 기다리지 말고 바로 GET한다. 」**<br><br>⚠ 🔴 **(2026-08-08 정정) 종전에는 여기에 「종단이 아닐 때만 통지를 기다린다」가 붙어 있었습니다 — 그 「기다린다」에 **끝이 없었습니다.** 배경 드레인 루프가 없어 **통지가 영영 안 올 수 있습니다**(아래 ⓖ). |
| ⓖ | 🔴 **배경 드레인 루프가 없습니다 — `queued`는 다음 POST 전까지 안 풀립니다** | **잡을 도는 유일한 계기가 「새 POST」입니다.** 실측(2026-08-08 · 전수):<br><br>`run_next()` 프로덕션 호출처 = **라우터 둘뿐**(`api/routers/counsel.py:687` · `api/routers/problem.py:336`) — 둘 다 **POST 핸들러 안**입니다. 배경 루프(`asyncio.create_task` · `while True` · 스케줄러) = **0건**.<br><br>🔴 **ⓐ GET 폴링으로는 안 움직입니다.** `GET /v1/counsel/drafts/{job_id}`는 `_refresh_view`가 `supervisor.get()`으로 **현재 phase만 읽습니다** — `run_next`를 부르지 않습니다. **BE가 폴링해도 잡은 그대로입니다.**<br>🔴 **ⓑ lease가 테넌트로 갈립니다** — `lease_next(tenant_id, worker_kind)`라 **다른 학원의 요청으로는 안 풀립니다.**<br>**ⓒ 언제 나나** — POST가 `run_next`를 부를 때 **`job_id`를 지정해 집을 수 없어** 같은 테넌트에 앞선 잡이 있으면 **그걸 집습니다.** 큐가 비어 있으면 자기 잡을 집으므로 **드뭅니다 — 다만 「안 난다」는 아닙니다.**<br>**ⓓ counsel·pg 둘 다 같습니다**(pg 쪽 기록은 `part_b/09` §2-24).<br><br>⚠ **v1 인라인 실행의 성질이지 결함 신고가 아닙니다** — 다만 **「넣으면 곧 돈다」로 읽으면 틀립니다.** 🔴 **운영에서 `queued`가 안 풀리는 것을 보시면 알려 주십시오** — 배경 드레인은 별도 작업이고 그 신고가 오면 우선순위를 올릴 수 있습니다(99 #21). |
| 🔴 **ⓗ** | 🔴 **AI 서버 재배포 금지 (2026-08-13 ~ )** | `develop`에 **evidence 확장**이 들어갔습니다(#260·#261 — `role`·`observed`·`sample_size`·`occurred_on` 신설, 근거 항목 수 3건 → 최대 6건). 🔴 **백엔드 DTO·마이그레이션 배포 전에 AI가 먼저 나가면 역직렬화가 깨집니다** — 저희가 *"①이 배포되기 전에는 저희가 안 나갑니다"* 라고 약속한 그 순서입니다. ⚠ **머지는 배포가 아닙니다** — 지금 막는 것은 **윈도우 AI 서버가 `develop`을 당겨 재기동하는 것**입니다. **해제 조건: 승우님 「DTO 배포 완료」 통보.** 그 전까지 윈도우는 `76d3527`(#260 머지) **이전 커밋에 고정**합니다. 🔴 **말로만 정하면 내일 다른 사람이 배포 명령서를 보고 그대로 돌립니다** — 그래서 여기 적습니다. |
...
□ ⓖ  🔴 `queued`가 오래 안 풀리면 **폴링·대기로는 안 풀립니다** — 알려 주십시오
□ ⓕ  🔴 202의 status가 종단이면 **통지를 안 기다리고 바로 GET**한다 — 안 그러면
     `template_only`·근거 0건 두 경로가 **영영 안 옵니다**
```
