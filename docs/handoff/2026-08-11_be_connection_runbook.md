...
| ⓕ | 🔴 **완료 통지가 안 오는 경로가 둘** · **202 바디는 언제나 2키** | **ⓐ 202는 `{job_id, status}` 둘뿐입니다** — 초안 실물은 **언제나 GET**입니다(`success_envelope(data={"job_id": …, "status": …})` · GET만 `view.model_dump()`로 `result`를 싣습니다). 04 §3.9도 `202 { "job_id": "cj_1029", "status": "queued" }`로 **2키가 맞습니다.** **ⓑ 🔴 `topic=schedule`(`template_only`)과 인용 가능 근거 0건(`rejected_insufficient`)은 완료 통지가 없습니다** — 그 둘은 `enqueue` **앞에서 반환**해 **`WorkerJob` 행 자체가 안 생기고**, 잡이 없으면 종단 phase 전이가 없어 **통지가 발생할 자리가 없습니다.** ⚠ **Kafka를 배선한 뒤에도 참입니다** — 「아직 미구현이라 어차피 다 안 온다」와 **다른 사실**입니다. **ⓒ 일반 경로는 동기입니다** — v1은 같은 요청에서 러너를 1회 돌리므로 **202가 이미 `succeeded`인 경우가 많습니다.** `queued`로 오는 것은 그 러너가 큐의 **다른 잡**을 집었을 때입니다(둘 다 정상). <br><br>🔴 **⇒ BE 분기 규칙 한 줄 — 이 하나로 세 경로가 다 덮입니다:**<br>**「202의 `status`가 종단(`succeeded`·`failed`·`cancelled`)이면 통지를 기다리지 말고 바로 GET한다. 종단이 아닐 때만 통지를 기다린다.」** |
