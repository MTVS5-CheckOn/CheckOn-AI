...
| 🔴 **라벨 제안**(`/v1/labels/suggest`) | **REST 동기 200 · 브로커 없음** | 🔴 **AI 가 만들어 주는 「추천」이라 이벤트로 흘러오지 않는다 — BE 가 묻는다.** 잡·`job_id`·완료 통지가 없고 **HTTP 응답이 유일한 통로**다(04 §3.7 · 8/22 판정). ⚠ 그래서 **재시도는 BE 몫**이고, 🔴 재시도 안전성은 아직 못 보증한다(99 #203). |
