...
| `paused` | 체크포인트에서 안전하게 중단되어 resume 가능. 🔴 **(8/19) counsel 축은 「기계는 있고 호출자가 없다」** — `Supervisor.resume`·`JobStore.resume` 는 있지만 **counsel HTTP 표면에 resume·cancel 엔드포인트가 0개**다(실측). 그리고 v1(N=1)에서는 이 phase 로 가는 경로 자체가 없다. ⚠ `problem_generation` 축은 이 조건과 무관하다 |
...
⚠ **HTTP 경계뿐 아니라 워커에도 같은 규율이 걸린다** — 비동기 잡은 예외를 **상태**(`paused`·`failed`)로 수렴시키므로 `except` 절이 더 많고, 그만큼 부수효과가 빠질 경로도 많다(8/7 실제: `_run_guarded`의 5개 절이 전부 적재를 안 지났다). 🔴 워커에서는 `finally` 안의 적재 실패가 **원인 예외를 교체**해 수렴 상태를 뒤집을 수 있다(`LlmCircuitOpenError` → `WORKER_INTERNAL` · `paused` → `failed`) — 실패 경로에서는 적재 오류를 삼키고 로그만 남긴다.
...
> 🔴 **(2026-08-19) 위 「상담팩 중단(paused)」 행은 v1 에서 아직 만들지 마세요.**
>
> - **v1(N=1)에서는 이 상태가 안 납니다.** 서킷 카운터는 **학생 단위**로 오르는데 counsel 은
>   문의 1건 = 학생 1명이라 카운터가 **최대 1**이고 임계는 **2 이상**입니다(기동 가드).
>   ⇒ `paused` 로 가는 경로가 **없습니다**(2026-08-19 실측).
> - 🔴 **`[이어서 생성]` 버튼이 부를 엔드포인트가 없습니다.** resume **기계**는 있습니다 —
>   `contracts/agents.py`(`assert_paused_resume`) · `Supervisor.resume` · `JobStore.resume`.
>   **없는 것은 HTTP 표면 하나**입니다.
> - **여는 조건: N>1**(월별 벌크·Kafka). 그때 표면을 열고 이 문구가 살아납니다.
>   ⚠ 문구를 지우지 않은 이유 — `part_a/06`·인박스 계약의 투영이라 근거가 있고, 지우면
>   N>1 이 왔을 때 다시 만들게 됩니다.
...
| 요청의 `type_tags`에 **예약 태그**(`apply`)가 있다 ✅ **구현됨(8/9)** | **400** | `type_tag_not_supported` | **v1 산출 축이 아니다**(99 ㊣ — 골든셋에 표본이 없어 검증 불가). 주체 3분할상 **호출자가 요청을 고쳐야** 하므로 4xx다 — 위 행과 같은 원칙이다. `detail`에 요청된 태그와 **v1 지원 목록**을 실어 BE가 무엇으로 바꿀지 알게 한다 — 실측: `{reason: "type_tag_not_supported", type_tags: ["apply"], supported: ["concept","critic","fact","infer"]}`. 🔴 **거절이 `enqueue.py::reject_unsupported_type_tags()`, 즉 잡 생성보다 앞이라 실패 잡이 남지 않는다**(99 #01 첫 사례). ⚠ **학습 이벤트(`LearningEvent.type_tag`)에는 `apply`를 보내도 된다** — 받아서 R6 집계·표시까지 한다. **막는 문은 출제 요청 하나뿐**이다 |

⚠ 위 `kind_not_implemented`와 **같은 원칙의 세 사례**다 — *"생성기가 없다 / 조용히 버리면
오해한다"*. 없는 기능을 200으로 받아 주는 것이 이 사전이 가장 경계하는 형태다.

⚠ **v1 제약의 상세(어떤 `area_tag`가 되는지)는 여기 적지 않는다** — 정본은
`04_api_contract.md` **§3.11**이다. **두 곳에 같은 목록을 두면 갈린다.**

> 🔴 **(8/7) 포인터 이동** — 종전 정본은 B의 `part_b/09_integration_proposals.md` §2-19.4였고,
> 그 절의 **구현분이 04 §3.11로 이관**됐다(B가 *"A 소유 정본 04에 반영"* 을 전제로 쓴 초안이다).
> 09에는 **`[구현 후속]` 7절만** 남는다 — 04는 그것을 **가리키기만** 하고 복제하지 않는다.
> ✅ **(8/9) B가 09 §2-19.4를 04 §3.11 포인터로 축소했다**(09 v4.1) — *"목록이 두 곳에 산다"* 던 임시 상태가 끝났다. **정본은 04 §3.11 하나**다.
> *"두 곳에 같은 목록을 두면 갈린다"* 는 이 판단이 **이미 두 번 맞았다**(§7 · 99 ㊩).
