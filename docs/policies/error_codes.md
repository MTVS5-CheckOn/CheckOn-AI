...
| 요청의 `type_tags`에 **예약 태그**(`apply`)가 있다 `[제안 · B 구현 대기]` | **400** | `type_tag_not_supported` | **v1 산출 축이 아니다**(99 ㊣ — 골든셋에 표본이 없어 검증 불가). 주체 3분할상 **호출자가 요청을 고쳐야** 하므로 4xx다 — 위 행과 같은 원칙이다. `detail`에 요청된 태그와 **v1 지원 목록**을 실어 BE가 무엇으로 바꿀지 알게 한다. ⚠ **학습 이벤트(`LearningEvent.type_tag`)에는 `apply`를 보내도 된다** — 받아서 R6 집계·표시까지 한다. **막는 문은 출제 요청 하나뿐**이다 |
...
> ⚠ **B의 09 §2-19.4 정리 전까지는 목록이 두 곳에 산다** — 정리는 B 몫이고 A는 09를 건드리지 않는다.
