...
class TypeTag(StrEnum):
    ...
    들어온다.** 문마다 답이 다르다(8/8 전수 실측):
      ...
      · **출제 요청**(BE → `ProblemRequest.type_tags`) — **400 `type_tag_not_supported`.**
        ...
        🔴 **오늘은 400이 아니다**(㊩ 병기 · 8/7) — **문 앞 검사가 없어** `POST`가
        **500**을 내고 잡은 만들어져 `failed`로 수렴하되 **`job_id`가 응답에 없다**
        (고아 잡 · 04 §3.11 · 99 ㊨).
        ⚠ **B의 문 앞 400이 들어가면 「오늘은 400이 아니다」 문단과 바로 위 `[제안]`
        줄을 함께 지운다** — 둘 다 그때 낡는다. ⚠ **(8/7 정정) 종전 표기는 「이 두 줄」
        이었는데 실제로는 네 줄이라 2줄만 지우면 조각이 남았다** — **줄 수를 세지 말고
        경계를 말한다.** 조건을 안 적으면 고쳐진 뒤에도 남아 **거짓이 뒤집힌 채로 산다**
    ...
    **여는 조건:** ㊛ 태깅 골든셋에 `apply` 표본이 쌓이고 baseline이 정해질 때.
    ...
    🔴 **출제 요청 미수용** `[제안 · B 구현 대기]` — **구현 전인 지금은 수용되고 500이
    난다**(99 ㊨ 8/7 실측: `POST /v1/problems`가 HTTP 500 `INTERNAL`을 내고 잡은
    `failed`로 수렴하지만 `job_id`가 응답에 없어 조회할 수 없다). 종전 문구
    *"출제 요청 미수용"* 은 **단정형**이라 구현된 것으로 읽혔다 — ㊩ 규칙을 만든 PR이
    그 규칙을 어긴 자리다. **미구현 계약은 약속한 동작과 현재 동작을 함께 적는다.**
