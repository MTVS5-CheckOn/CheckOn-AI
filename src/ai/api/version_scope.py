...
#:
#: 🔴 **`engine`은 `"app-0.1"`로 확정한다(8/10 · 99 ㊙ ⓐ).** `VersionSet`의 네 필드가
#: 필수라 비울 수 없고, 이 토큰은 **임시값이 아니라 *"엔진이 관여하지 않은 응답"* 을 뜻하는
#: 명시값**이다. 근거 셋:
#:   ⓐ 빈 문자열은 *"못 받았다"* 와 구분되지 않는다(`run_store.UNKNOWN_MODEL`과 같은 판단)
#:   ⓑ `engine`을 nullable로 바꾸는 안은 `contracts/execution.py`(**양자 승인**)를 열고
#:      04 §2.2의 *"공통 6종"* 정의까지 바꾸는데, **실 피해가 없다** — 이 응답은
#:      원장에 남지 않는다(등록 안 된 경로는 애초에 이 폴백에도 안 온다 · 위 참조)
#:   ⓒ FastAPI의 `version="0.1.0"`을 그대로 쓰는 안은 **앱 릴리스 버전이지 엔진이 아니라**
#:      또 다른 거짓말이 된다
#: ⚠ **근거를 여기 옮겨 적은 것이 8/10 작업이다** — 종전에는 99에만 있어 **코드만 보면
#: 임시값으로 읽혔고**, 다음 사람이 *"정하면 바꿔야 하는 값"* 으로 취급할 자리였다.
