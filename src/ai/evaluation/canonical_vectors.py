...
#:
#: 🔴 **왜 갈리나 — [실측 2026-08-21] 추정이 아니다.** 백엔드 실물에서 났다:
#:     저희가 내보낸 값   `"generated_at": "2026-08-21T05:59:50.869920Z"`  6자리
#:     백엔드 응답에서    `"generatedAt":  "2026-08-21T05:59:50.86992Z"`   🔴 5자리
#: 재직렬화에서 **후행 0 이 잘렸다**(Java `OffsetDateTime.toString()` 의 트림).
#: 규칙 문서 §2-1 ㉡ 과 이 벡터가 **가정으로** 세운 것이 그대로 났다.
#: ⚠ `generated_at` 은 **해시 표면이 아니라** 지금 사고는 없다 — 🔴 다만 **해시되는 필드에
#: 소수 초가 하나라도 실리면 그때 갈린다.** 그래서 이 벡터가 그 자리를 미리 든다.
#: ⚠ **벡터 값은 안 바꿨다** — 근거가 세졌으므로 **문면만** 올렸다(로그 146 의 반대 방향:
#: 「근거가 약한데 [읽음]이라 적지 마라」가 아니라 「세졌으면 올려라」다).
...
**[`../../15_canonical_serialization.md`](../../15_canonical_serialization.md)**

⚠ **어느 벡터가 어느 절을 찌르는지는 그 문서 §3 에 표로 있다.** 해시가 갈렸을 때
*"어딘가 다르다"* 에서 멈추지 말고 **그 표에서 절을 찾아** 그 절의 문면을 심판으로 쓴다.
🔴 **어느 쪽 구현도 심판이 아니다** — 먼저 낸 쪽으로 상대가 맞추면 같은 문제가 한 칸
뒤로 옮겨질 뿐이다.
