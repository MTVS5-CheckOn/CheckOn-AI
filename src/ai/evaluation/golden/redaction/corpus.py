...
CORPUS: tuple[RedactionCase, ...] = (
    ...
    # ── 75~76 학원 핵심 어휘(오탐 검증 — 유지돼야 함) ────────────
    # 🔴 99 #123 **제목의 낱말**인데 №54 가 닫을 때 안 들어갔다(8/22 정정 · №65).
    #   `whitelists.name_exclude` 의 `문제집`·`성적표` 가 이 둘을 막는다.
    RedactionCase(
        id=75,
        category="함정·학원어휘(문제집)",
        text="문제집을 바꾸는 게 좋을까요",
        expected="문제집을 바꾸는 게 좋을까요",
        must_present=("문제집을",),
        context_trap=True,
        reason=(
            "🔴 `문`(성씨)+`제집`+조사 `을`. **학부모 문의 20건 표본에서 2건이 이 형태였다**"
            "(№64 §C) — 걸리면 라벨 제안이 **500 INTERNAL** 로 죽는다(99 #192)"
        ),
    ),
    RedactionCase(
        id=76,
        category="함정·학원어휘(성적표)",
        text="성적표를 언제 받을 수 있나요",
        expected="성적표를 언제 받을 수 있나요",
        must_present=("성적표를",),
        context_trap=True,
        reason=(
            "🔴 `성`(성씨)+`적표`+조사 `를`. ⚠ **99 #123 이 제목에 건 낱말**인데 №54 가 "
            "«형태를 고쳤으니 닫혔다» 로 판정해 빠졌다 — 안건이 건 것은 **낱말**이었다"
        ),
    ),
)
