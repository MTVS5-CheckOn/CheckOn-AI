...
class CounselSettings(BaseSettings):
    ...
    counsel_inline_drain_max: int = 0
    ...
    **전부** 따라 움직인다. 그 연쇄를 사람이 기억으로 좇지 않도록 검사로 묶었다(㉪).
    ...
    ━━ 🔴 2026-08-20 · K 1 → 0 — **응급처치를 걷었다** ━━

    ⚠ **0 은 위 역산식의 결과가 아니다** — ⌊480 / 450⌋ = 1 이다. 식은 **상한**을 주고
    그 안에서 값을 고른다. 0 은 별개 판단이다:
      ⓐ 배경 드레인(`composition/counsel/drain.py`)이 배포에 떠 있고 DB 폴링이
        관측된다(하트비트 `sweeps` 60→118→176 · 99 #137).
      ⓑ K≥1 은 **요청 스레드가 잡을 끌고 간다** — POST 예산이 잡 최악에 묶이고
        04 §2.4 예산·Cloudflare 상한(99 #115)이 둘 다 요청 경로에 걸린다.
      ⓒ K=0 은 **요청당 `AsyncPostgresSaver` 커넥션을 없앤다**(99 #127 셋째 항).
    🔴 **되올릴 때는 식으로 다시 계산하라 — 「일단 1」로 올리지 마라.**
    ⚠ 대가: POST 가 `succeeded` 를 안 돌려준다. `queued` + `Retry-After` 폴링이 정본이다
      (거부·템플릿 단축 경로만 잡 없이 `succeeded` 로 즉답한다 — 04 계약).
