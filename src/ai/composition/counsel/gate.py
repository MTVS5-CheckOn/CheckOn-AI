...
def check_counsel_gate(
    ...
    **명시**한다 — 숨은 옵트아웃이 아니라 보이는 선언이다.
    ...
    #:   `이해력`·`다른` 을 넣어 `redact()` 오탐을 닫았다(99 #123 · #83 · №54).
    #:   ⇒ **방어가 한 층에서 두 층이 됐다**(프롬프트 지시 + 이 게이트). 🔴 **이 층의 값은
    #:   줄지 않았다** — LLM 은 지시를 **안 따를 수 있고** 여기가 **마지막** 방어다.
    #:   위 문면은 «유일했던 시절» 의 기록으로 남긴다.
    #:
    ...
    if buffered:
        ...
        #: 종전 근거는 **«못 싣는다»**(기술적 제약)였다. №54 가 그 오탐을 닫아 **실을 수는
        #: 있게 됐다.** 그런데도 **안 싣는다** — 이제 **판정**이다(8/21 · 진희):
        #:   ① 🔴 게이트 사유는 **응답·로그를 타고 나간다.** `이해력이 부족` 은 **낙인
        #:      표현**이고, 사유에 담으면 **새 노출 축**이 열린다 — 막으려던 말을
        #:      «막았다는 이유로» 실어 보내는 셈이다.
        #:   ② №54 는 **오탐 처방** 회차다. «이제 가능하니 하자» 는 별개 설계 결정이고
        #:      그 물음(«사유에 무엇을 담나»)은 **아직 아무도 안 물었다.**
        #: 🔴 위의 **«#102 와 같은 사고»** 는 남긴다 — 다른 어휘에서 **여전히 참인 위험**이다.
