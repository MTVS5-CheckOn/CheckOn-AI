...
🔴 **위 「미확정」은 8/9에 이미 닫혔다**(99 ㊼ ✅) — 근거는 `gpt-5.4-mini`에서
**`temperature=0.0`과 `seed`를 둘 다 고정**하고도 출력이 갈린 8/7 4차(358 vs 361자 ·
둘 다 비지 않음)다. **서버의 seed는 best-effort이고 존중되지 않는다.**

🔴 **8/13 실측은 그 위에 하나를 더한다 — 모델을 바꿔도 재현 축이 없다.**
`gpt-5.6-luna`에 같은 seed로 8회 보내 **문장 3종**(6/1/1) · `system_fingerprint`는 `None`.
⚠ **다만 이 회차는 원인을 귀속하지 못한다** — 온도가 없는 상태(모델 기본)라 seed 무시인지
온도 때문인지 안 갈린다. 이 모델은 `temperature=0.0`이 400이라 **분리 실험이 불가능**하다.
⇒ **다음 모델로 옮길 때 `seed`를 먼저 버리지 마라.** 온도를 되살려도 이 모델에서는 400이라
얻는 것이 없지만, **다른 모델에서는 조건이 달라진다.**
