...
"""브리핑 1건에 허용하는 **천장**. 사용량 추정치가 아니다.

🔴 **줄이지 마라.** 관측 사용량에 맞춰 조이면 2026-08-13 사고가 재발한다.

## 왜 천장이지 예산이 아닌가

`max_completion_tokens`는 예약이 아니라 **상한**이다 — 실제로 안 쓰면 비용이 발생하지
않는다. 따라서 **여유를 크게 두는 비용은 0이고, 조이는 이익도 0이다.** 이 상수가 막는
것은 요금이 아니라 **폭주**다.

⚠ 종전 문면은 *"한 문장이라 상한을 좁게 준다 — 과생성·지연을 막는다"* 였다. 그 말이
**이 상수를 사용량 추정치로 읽게 만들었고**, 그래서 128이 합리적으로 보였다.

## 2026-08-13 사고 (99 #54)

`gpt-5.6-luna`는 **추론 모델**이다. 추론 모델에서 `max_completion_tokens`는 출력이 아니라
**추론 + 출력**을 함께 묶는다.

- **128 시절**: reasoning이 128을 전부 소진 → `content=""` · `finish_reason="length"`
  → 어댑터가 `ParseFailed`로 올림 → 브리핑이 **상시 템플릿 폴백**
- **실측**(원장 정본 프롬프트 818자 · prompt 537토큰 · 2회):
  reasoning **89 · 96** / 출력 **41 · 40** / 합계 **130 · 136** / `finish_reason="stop"`
- 성공하던 다른 경로(상담·문항생성)는 `deterministic_params()`가 `max_tokens=None`을 주어
  **키 자체를 안 보낸다.** 🔴 **크기 차이가 아니라 유무 차이였다.**

## 이 숫자가 보장하지 않는 것

실측은 **R1 브리핑 1종**이다. R2·R3·R6·병합 신호는 facts가 많아 추론이 늘 수 있고
**추론량에는 상한이 없다.** ⇒ **1024도 절대 안전이 아니다.**

🔴 **재발을 보이게 하는 것이 본질이다** — 절단(`finish_reason="length"`)을 `parse_fail`과
구분해 원장에 남기는 항목(99 #54)이 **이 상수보다 중요하다.** 지금은 절단과 파싱 실패가
같은 글자로 남고, 그 호출의 토큰은 `0/0`으로 사라진다.

## 출력이 폭주하지 않는 이유

브리핑 문장은 **게이트가 `MAX_BRIEF_LENGTH`자로 막는다.** 천장을 키워도 과생성 위험이
없다(2048로 재도 `stop` · 출력 41토큰).

## 이 문서가 만료되는 조건

모델이 비추론 모델로 바뀌어도 **이 값을 되돌리지 마라** — 천장의 성질은 같다.
줄이려면 위 「왜 천장인가」를 **먼저 반증하라.**
"""
