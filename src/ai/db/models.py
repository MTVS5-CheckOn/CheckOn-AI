...
class LabelConfirmationStat(Base):
    ...
    🔴 **왜 테이블인가** — 강사의 라벨 확정은 «**어느 축의 제안이 얼마나 뒤집히나**» 를
    세는 **우리 모델의 성적표**다. 종전에는 프로세스 안 카운터였고 🔴 **재배포마다 0** 이
    됐는데 근거는 **한 달 단위**로 쌓여야 한다(실측 · 99 #239).
    ⚠ 🔴 **(2026-08-25 정정) 「군집의 착수 근거」가 아니다.** #443 이 그렇게 적었는데
    🔴 **실측으로 틀렸다**(99 #250): 군집이 필요한 것은 «4축이 X% 를 못 담는다» 이고,
    그건 **요청 단위**(분모=요청 수 · 분자=제안 0건 수)라 이 테이블이 담을 수 없다 —
    `rejected` 는 «이 제안이 틀렸다» 이지 «이 축이 못 담는다» 가 아니고, 제안 0건은
    확정이 안 와 **아무것도 안 남는다.** 🔴 **이 테이블은 그대로 둔다** — 위 물음에는
    답한다. 🔴 **양자 파일에 손댄 사유: 거짓 문면을 참으로 고치는 주석 한 줄.**
