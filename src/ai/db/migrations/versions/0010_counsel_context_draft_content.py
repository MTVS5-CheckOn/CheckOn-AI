...
🔴 **`content`를 `NOT NULL`로 세운다.** 0010 시점에 `draft` 행이 **0건**이다 —
`Draft` ORM을 참조하는 코드가 정의 한 곳뿐이라 **쓴 적이 없다**(2026-08-12 전수 + 실 PG
실측). ⚠ 그래서 `server_default`를 두지 않는다: 빈 문자열 backfill은 *"초안이 있다"* 는
거짓 데이터이고, 그러면 게이트 거부와 정상 초안이 **같은 값**을 갖는다. 0007이 같은
「행 0건 전제」로 NOT NULL을 세운 선례다.
