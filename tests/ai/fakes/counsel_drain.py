"""🔴 **이 파일은 fake 가 아니다.** `fakes/` 에 있는 이유는 `pythonpath` 하나뿐이다.

`drain_once` 는 프로덕션 `runner.run_next` 를 **그대로 부르는 다리**다 — 라우터의 인라인
루프(`api/routers/counsel.py::_generate`)와 배경 워커(`composition/counsel/drain.py`)가
부르는 **같은 함수**다.
⚠ **여기에 테스트 전용 분기를 넣지 마라.** 넣는 순간 이 파일을 쓰는 검사가 **프로덕션을
안 잰다**(결정 로그 127 · 그 사실을 잠그는 검사가 `integration/test_drain_once_bridge.py` 에 있다).
🔴 **이름이 먼저 읽힌다** — 검사가 못 박고 있어도 «`fakes/` 니까 프로덕션과 무관하겠지» 로
읽힌다(99 ㊒ *"이름이 이미 거짓말이다"*). 그래서 이 문단이 첫 줄에 있다.
