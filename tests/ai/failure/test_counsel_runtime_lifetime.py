...
def test_the_job_ledger_is_not_capped_and_that_is_deliberate() -> None:
    """🔴 **잡 원장에는 같은 처방을 쓰지 않았다** — 두 가지 이유다(99 ㊐).

    ⓐ **소유는 A 단독인데 소비자가 둘이다**(8/7 B 회신). B가 pg 워커에서
      `build_agent_job_store()`를 그대로 부르기로 확정했으므로 `store_backend=memory`에서
      counsel 잡과 problem_generation 잡이 **같은 싱글턴**에 산다 — **A가 정한 숫자가 B
      경로의 잡을 지운다.** 소유권은 *"고쳐도 되는가"* 를 답하지만 *"혼자 정해도 되는가"* 는
      답하지 않는다.
    ⓑ **캐시와 원장은 애초에 처방이 다르다.** 밀려난 캐시 항목은 다시 만들면 되지만,
      밀려난 **잡**은 *"없어진 잡"* 이다 — GET이 404를 내고 BE는 그 잡이 존재한 적 없다고
      읽는다(존재 은닉과 구분 불가). paused 잡이 조용히 사라지면 강사가 다시 눌러 초안이
      2개 생긴다.

    ⚠ 무한 증가가 괜찮다는 뜻이 아니다 — **A가 단독으로 정할 수 없다**는 뜻이다.
    """
    import inspect
