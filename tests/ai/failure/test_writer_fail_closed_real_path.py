...
#: 막지 않는다). 종전 문면은 후보 1개짜리라 구현이 스펙보다 넓게 막던 시절에만 걸렸다 —
#: 이 파일이 미리 적어 둔 *"다시 골라야 한다"* 를 그대로 따랐다. 검사의 의도(「불확실한
#: fact 는 전송 전에 멈춘다」)는 그대로다.
_UNCERTAIN_SUMMARY: Final = "서연이가 민준이랑 같은 방향으로 가고 있습니다"
...
def test_no_gate_feedback_blocks_the_assembled_prompt() -> None:
    ...
    ⚠ 🔴 **(8/21) 위 예는 더 이상 안 걸린다** — `name_exclude` 에 `다른` 이 들어가 그 항의
    오탐이 닫혔다(99 #123 · №54). 실측: 단독도 결합도 **findings 0**.
    🔴 **그런데 이 검사의 근거는 그대로다** — 재는 것은 «그 어휘» 가 아니라 **«층이 다르다»**
    이고, 밀도 임계(후보 2개 이상 → 문장 통째)는 **결합에서만** 성립한다. 예가 낡은 것이지
    **성질이 낡은 것이 아니다.** 옛 예는 «왜 이 층을 따로 재기로 했나» 의 기록으로 남긴다.
