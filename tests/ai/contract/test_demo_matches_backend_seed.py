...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "② type_bias 2 vs 시드 기대 1 — 🔴 원인은 **데모 구성에 R6 편중 학생이 하나 더 "
        "있는 것**이다(데모 st_08·st_11 / 시드 st_04 하나). ⚠ skip 이 안 만들어져서가 아니다 "
        "— 그건 별개 사실이다: 시드 st_14 가 「R6 skip · tagging_below_60pct」를 겨냥하는데 "
        "StudentPlan 에 태그 누락 손잡이가 없어 **데모에는 대응하는 학생이 아예 없다**(99 #67). "
        "🔴 데모의 R6 하나는 capped_out 경로를 태우려는 정원 채우기라 「갈림」이 아니라 "
        "「목적 차이」일 수 있다 — 어느 쪽을 맞출지도 미정이다. "
