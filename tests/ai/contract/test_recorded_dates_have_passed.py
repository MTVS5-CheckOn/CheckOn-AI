...
*"8/10 해소"*·*"(8/11 실측)"* 처럼 **일어난 일의 시점**이 아직 오지 않은 날짜면 거짓이다.
...
#: **파일 + 사유**로 등재한다. ⚠ `docs/handoff/…_pg_status_briefing.md`는 **B 소유 문서**라
#: 문면을 고칠 수도 없다(A가 남의 핸드오프를 고치지 않는다).
_PLAN_TABLES: Final = {
    "docs/handoff/2026-08-08_pg_status_briefing.md": (
        "B의 일정표 — 행이 `| **8/10** | 영속화 왕복 테스트 …` 형태라 계획임이 표 머리에 있다. "
