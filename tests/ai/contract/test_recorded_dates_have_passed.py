...
def _newest_commit_date() -> datetime.date:
    ...
    🔴 **그리고 뒤집기·측정 결과에 「미래 날짜」를 그대로 적지 마라** — 상대 표현으로
    적어라(«다음 주 월요일» · «버킷 둘»). 실측 8/25: #443 이 주 버킷 뒤집기 결과를
    `2026-08-31` 로 적었고 이 가드가 **미래 날짜**로 잡아 develop 이 red 였다.
    ⚠ 🔴 #234 와 **갈래가 다르다** — 그건 «커밋 전에 돌렸다» 이고 이건 «**본문에 미래
    날짜를 적었다**» 다.
