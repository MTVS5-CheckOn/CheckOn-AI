...
#: ⚠ **한계 셋을 적는다 — 이 검사가 무엇을 못 보는지가 검사의 절반이다**(로그 85):
#:   ⓐ 구간 안의 PR 번호(예: `PR #15`)를 인용하면 **안건으로 오해**한다. 실측으로 그런 자리는
#:      없고 생기면 red가 나서 알게 된다.
#:   ⓑ 🔴 **구간 밖 번호(`#77`)는 못 잡는다** — PR로 보기 때문이다. 실측(8/9): 등재가
#:      `#01`~`#31` **연속**이라 **구간 내 빈 번호가 0**이다 ⇒ **지금 이 검사가 실제로 잡는 것은
#:      접미 형태(`#31′`)와 앞으로 생길 빈 번호**다. 얇지만 **`#14′`를 잡은 것이 그 값**이었다.
#:   ⓒ 등재가 늘면 구간이 넓어져 **PR 번호와 겹칠 수 있다** — 그때 ⓐ가 실제 문제가 된다.
_NOT_AN_ITEM_REF: Final = {
    "PR #": "GitHub PR 번호 — `PR #156` 형태로 앞에 `PR`이 붙는다",
    "PR-": "PR 별칭과 함께 쓰인 자리",
    "issue": "이슈 번호",
    "#142": "PR 번호 — 기호 충돌 사고를 낸 그 PR(로그 79)이라 자주 인용된다",
    # 🔴 **폐기된 표기를 인용하는 문면** — 날짜 가드의 `오기`와 같은 ⓓ 논리다:
    #   *"종전 표기는 `#14′`였고 그 등재행은 0건이었다"* 는 **그 표기가 틀렸다고 말하는 문장**이라
    #   «그 안건을 가리킨다»는 인용이 아니다. ⚠ 안 두면 **정정을 적을 수 없다**(적는 순간 red).
    "종전 표기": "ⓓ 폐기된 표기를 인용해 정정하는 문면",
    "자리표": "ⓓ 같은 것 — 「아직 없는 안건의 자리표」를 설명한다",
    "등재행": "ⓓ 같은 것 — 「등재행이 0건이었다」를 설명한다",
}
            ...
            for number, line in enumerate(
                ...
                for match in _NUMBER_REF.finditer(line):
                    ...
                    if ref in registered:
                        continue
                    #: ⚠ **창을 ±30으로 둔다** — ±12/+6이었을 때 표식이 **뒤에** 있는
                    #   자리를 놓쳤다(*"`#14′` → #31(등재행 0건이던 자리표)"*). 좁히면 미탐이다.
