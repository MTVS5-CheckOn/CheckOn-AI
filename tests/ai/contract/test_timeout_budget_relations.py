...
def test_the_out_of_pool_connection_terms_are_named_and_bounded() -> None:
    ...
    ⚠ 🔴 **「상한이 있다」는 코드 구조 판단이고 실측이 아니다(8/21).** 세 항이 모두
    설정·구조로 묶인 것은 **소스에서 확인**했지만, **운영 DB 커넥션 실측은 아직이다** —
    **99 #127** 이 그 자리이고 **동시 300건 재현**(8/20 조건)으로 A·B 가 **같이** 잰다.
    ⚠ 종전 이름은 `..._even_though_it_is_unbounded` 였다. **코드가 바뀌어 이름이 바뀐 것이
    맞다** — 다만 **「닫혔다」가 「쟀다」로 읽히지 않게** 이 줄을 남긴다(로그 173).
    """
    import inspect
