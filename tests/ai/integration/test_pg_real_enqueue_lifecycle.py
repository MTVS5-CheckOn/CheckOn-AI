...
#: ⚠ 🔴 **(8/22) G3-C(`mapping_probe`) 절을 통째로 뺐다** — import 축 개발 중단으로
#: `MappingProbeRunner`·probe 저장소가 사라졌다(99 #187). 이 절이 재던 것 넷:
#: 원장 결속 · `agent_run`↔`step`↔`spec` 한 실행 묶임 · 테넌트 격리 · 남의 프로파일 거부.
#: 🔴 **그 성질들은 counsel·problem_generation 축이 같은 파일에서 계속 잰다** — 사라진 것은
#: **세 번째 축의 사례**이지 성질이 아니다(감사 검사도 3건 → 2건으로 줄었다).
...
def test_all_three_capabilities_audit_together_in_one_tenant() -> None:
    ...
    async def scenario(sessions: async_sessionmaker[AsyncSession]) -> None:
        ...
        assert len(findings) == 2, f"두 실행이 안 보인다: {len(findings)}건"
        ...
        #: ⚠ 🔴 **(8/22) 셋 → 둘.** probe 축이 사라졌다(99 #187) — 종전엔 셋 다 `ok`였고
        #: 그 전에는 조사만 `separate_gap` 이었다(99 ㉾). **그 이력은 여기 남긴다.**
