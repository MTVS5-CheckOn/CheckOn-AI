...
**에이전트가 눈으로 알아채 즉시 되돌렸다.** 🔴 **눈에 의존하는 방어는 방어가 아니다.**
...
관문은 **조립 경로**에 서야 한다(99 #58 · PR-π · `llm/` 소유). 스크립트마다 가드를 다는
길은 실측으로 폐기했다 — `create_app()`을 부르는 자리가 **137곳**이고, 그중 하나만
막으면 다음 자리가 남는다. 이 검사는 **관문이 서기 전에 오염이 커밋되는 것**만 막는다.
"""
...
#: 🔴 **글롭이다 — 목록을 하드코딩하지 않는다.** 새 산출물이 생기면 자동으로 포함되어야
#: 하고, 손으로 적으면 **추가한 사람이 이 파일을 모른다.**
...
#: 이 문장이었다**(`git show ea338a1:…/counsel/provider.py` → `_DEFAULT_FAKE_TEXT: Final =
#: "이번 주 학습 상황을 정리해 드립니다."`). 이후 `536c04d`가 `gate_floor_draft`로 확장해
#: 현재 fake는 훨씬 길다. ⇒ **낡은 것이지 실물이 아니다.**
#:
#: 🔴 **만료 조건이 걸려 있다** — `test_the_legacy_prose_registry_has_no_dead_entries`가
#: 「아무 산출물도 안 쓰는 등재」를 red로 만든다. counsel 데모를 재생성하면 지우게 된다.
...
def test_the_artifact_glob_actually_matches_something() -> None:
    ...
    (`test_llm_provider_pin`이 자기를 통과시킨 것 · 99 #57).
    """
    artifacts = _artifacts()
    assert artifacts, f"산출물을 하나도 못 찾았다 — 글롭이 틀렸다: {_ARTIFACT_GLOBS}"

    prose = [line for path in artifacts for line in _prose(json.loads(path.read_text("utf-8")))]
    assert prose, "산출물은 찾았는데 문면이 0건이다 — 키 이름이 바뀌었나"


@pytest.mark.parametrize(
    "artifact", _artifacts(), ids=lambda p: p.name
)
def test_committed_artifact_carries_only_fake_prose(artifact: Path) -> None:
    ...
    `LLM_PROVIDER=fake`를 명시해 재생성하라 — `.env`는 실물을 가리킬 수 있다(99 #58).
    """
...
def test_the_legacy_prose_registry_has_no_dead_entries() -> None:
    ...
    산출물이 재생성되면 그 문면은 사라지는데, 등재만 남으면 **다음 사람이 그 예외를
    사실로 읽는다.** 아무 산출물도 안 쓰는 등재는 red로 만들어 지우게 한다
    ...
    assert not dead, (
        f"등재된 낡은 문면을 쓰는 산출물이 없다 — 재생성됐으면 등재를 지워라: {dead}"
    )
