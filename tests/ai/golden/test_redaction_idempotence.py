...
def test_corpus_is_idempotent_end_to_end() -> None:
    ...
    2026-08-06 최초 실측 **예외 0건**(당시 코퍼스 54건).

    ⚠ **(8/8) 현재 크기를 여기 적지 않는다.** 종전 문면은 *"전 54건 예외 0건"* 이었고
    **코퍼스가 그 뒤 늘었는데 수만 낡아** *"코퍼스는 54건"* 으로 읽혔다(#02 부류).
    이 검사는 `CORPUS` **전건**을 훑으므로 수를 재진술할 이유가 없다 —
    **가드 없는 재진술은 또 갈린다.**
    """
    offenders: list[str] = []
    for case in CORPUS:
